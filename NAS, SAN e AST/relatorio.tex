...
\item essa versão futura chegou: o \textbf{macOS 27 ``Golden Gate''}, cujos \textit{betas} de
      desenvolvedor, a partir de junho de 2026, \textbf{já não incluem o cliente AFP}, com o efeito
      colateral de encerrar o suporte a \textit{backups} do Time Machine em Time Capsules e AirPort
      Disks \cite{macos27}.
\end{itemize}
...
localizou a cobertura de junho de 2026 documentando a ausência do cliente AFP nos \textit{betas} do
macOS~27 \cite{macos27}, além do próprio aviso exibido pelo macOS~26 Tahoe sobre o fim do suporte a
Time Capsule na versão seguinte.

\smallskip
\textbf{Ressalva de proveniência, mantida:} não localizamos comunicado formal de imprensa da Apple
declarando a remoção. A evidência é (i) o texto de depreciação da própria Apple \cite{appleafp},
(ii) o aviso do sistema operacional no macOS 26 e (iii) a verificação dos \textit{betas} do macOS 27
reportada pela imprensa especializada \cite{macos27}. \textbf{Categoria:} (ii) é fonte primária;
(iii) é medição publicada por terceiro.}
...
Este é o argumento quantitativo central a favor da SAN: \emph{a rede deixou de ser o gargalo}. Não é
um argumento a favor de gastar mais em FC; é a explicação de por que a alternativa Ethernet passou
a ser viável.}
...
\textit{campus} e área metropolitana, e a replicação intercontinental sobre FCIP é obrigatoriamente
assíncrona --- com a consequência de aceitar um RPO (\textit{Recovery Point Objective}) maior que
zero. \textbf{Categoria:} derivação nossa a partir de constante física.}
...
cliente depreciado no macOS 15.5 (mai./2025) \cite{appleafp} e ausente nos \textit{betas} do macOS 27
(jun./2026) \cite{macos27} & \textbf{Nenhuma.} Sem travamento em intervalo de \textit{bytes}
...
assincronia (ver derivação da Seção~\ref{sec:fcip}) \\
...
\textsc{delete}) \\
Atualização parcial & Sim, qualquer bloco & Sim, qualquer \textit{offset} & \textbf{Não}: o objeto
é substituído inteiro \\
...
\cite{awss3faq}. \textbf{Derivação nossa para dar escala a esse número:} com 11 noves, a
expectativa é de perder $10^{-11}$ dos objetos por ano; para $10^{7}$ objetos armazenados, o
número esperado de perdas é $10^{7} \times 10^{-11} = 10^{-4}$ por ano, isto é, uma perda esperada
a cada 10.000 anos. \textbf{Ressalva importante de rótulo:} isso é durabilidade
\emph{projetada} (do inglês \textit{designed for}), não medida --- e não protege contra exclusão
acidental nem contra \textit{ransomware}, apenas contra falha de mídia.
\end{itemize}
...
\begin{enumerate}[leftmargin=0.9cm,itemsep=2pt]
\item \textbf{Sem atualização parcial.} Alterar uma linha exigiria reescrever o objeto inteiro. Um
...
\textbf{Escolha pela unidade de abstração que o \textit{Database Engine} precisa controlar, não
pela velocidade do enlace.} Se o motor precisa ser dono da ordenação de escrita, da atomicidade da
página e do travamento --- OLTP transacional, \textit{cluster} com armazenamento compartilhado ---
ele precisa de \textbf{bloco}, e portanto de SAN. Se o motor pode delegar essas garantias porque a
carga é de leitura, de varredura ou de ingestão em lote, então \textbf{arquivo} basta, e o NAS
entrega o mesmo resultado por uma fração do custo.}

O corolário quantitativo vem da derivação da Seção~\ref{sec:topologias}: a rede FC contribui com
menos de 5\% da latência de um acesso NVMe. \textbf{A rede deixou de ser o gargalo}, e portanto o
argumento ``FC é mais rápido'' perdeu força; o que resta como diferença real é a
\emph{semântica}, não a velocidade.
...
\midrule
OLTP crítico, alta taxa de \textit{commit} & \textbf{SAN} (FC ou iSCSI em rede dedicada) &
A latência de \texttt{fsync} do \textit{log} governa a vazão de transações. Bloco cru elimina a
camada de sistema de arquivos remoto do caminho crítico e dá ao motor o controle da ordenação. \\
\addlinespace
...
\addlinespace
Banco relacional de médio porte, OLTP moderado & \textbf{NAS ou SAN}, indiferente &
Com NFSv4.1 e \texttt{O\_DIRECT} (ou dNFS \cite{oracledn}), a diferença é operacionalmente
irrelevante. Decida pelo custo e pela competência da equipe. \\
\addlinespace
\textit{Data warehouse}, OLAP, varredura sequencial & \textbf{NAS} & O custo dominante é banda, não
latência: um acesso já amortiza o custo de rede sobre megabytes. O NAS entrega a mesma banda por
menos, e o compartilhamento nativo entre nós de processamento é uma vantagem, não um problema. \\
\addlinespace
...
com cartucho ejetado, para o arquivo de longo prazo e como única cópia genuinamente inalcançável
por um atacante com credenciais válidas. O item (iii) é o que a nuvem, por construção, não
oferece.}
...
\bibitem{macos27} MACRUMORS. \textit{macOS 27 Golden Gate Kills Time Capsule Support}. 17 jun. 2026.
Cobertura da ausência do cliente AFP nos \textit{betas} do macOS 27. Disponível em:
\url{https://www.macrumors.com/2026/06/17/macos-27-golden-gate-kills-time-capsule-support/}.
...
\promptbox{P6 --- primeira rodada adversarial}{48 afirmações, 16 correções}{%
...
escrevesse mais. Os \textit{prompts} P6 e P8 produziram, juntos, 31 das 33 correções da
Seção~\ref{sec:erros}.}
...
\bottomrule
\endfoot
1 & ``A versão de remoção do cliente AFP ainda não foi anunciada.'' & Verdadeiro em mai./2025,
falso em set./2026: o macOS 27 já não traz o cliente AFP nos \textit{betas} de jun./2026
\cite{macos27}. & Rodada 1 & Gabriel Schmitz \\
