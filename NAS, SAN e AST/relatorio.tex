...
o custo de cada acesso, a semântica de travamento, o comportamento sob falha e o próprio conjunto de
...
O quarto é a \textbf{semântica de travamento previsível}. Em configurações de \textit{cluster} com
armazenamento compartilhado, como o Oracle RAC, vários nós escrevem no mesmo conjunto de blocos, e o
travamento tem de permanecer correto sob falha de nó e sob partição de rede.
...
travamento de arquivo'', e que ``alternativamente, as aplicações podem abrir seus arquivos com a
...
UTF-8 com decomposição canônica trafegam como atributos de primeira classe. Por fim, o travamento por
...
transparente, e um travamento amarrado à sessão. Comparado ao SMB~3 (Seção~\ref{sec:smb}) e ao
...
características acima: sem travamento independente de sessão, sem RDMA e sem disponibilidade
...
de fluxo. Registramos que a primeira versão deste relatório ignorava essas duas ressalvas e
apresentava como falhas do livro dois pontos que ele já cobre (Seção~\ref{sec:adversarial2}).
...
O NVMe-oF (\textit{NVMe over Fabrics}) transporta comandos NVMe sobre FC (FC-NVMe), TCP (NVMe/TCP)
ou RDMA (NVMe/RoCE). As filas paralelas pertencem ao modelo NVMe, e o SCSI não deve ser confundido
com a fila única por porta do AHCI. O transporte e a implementação também influenciam o desempenho.
...
\textbf{Legado, sem recomendação para novas implantações.} Possui travamento por intervalo de bytes, mas não identificamos configuração de SGBD homologada neste estudo \\
