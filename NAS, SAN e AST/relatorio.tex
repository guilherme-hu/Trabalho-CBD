...
demais diferenças: quem é dono do sistema de arquivos, onde ocorre o lock, quem garante a
...
com uma quinta categoria legítima: \emph{não encontrado}. O trabalho passou por três rodadas de
verificação adversarial documentadas na Seção~\ref{sec:postmortem} --- a última delas dedicada a
conferir, frase a frase, se cada citação está de fato no documento a que é atribuída --- e registra
\textbf{quatro divergências no material-fonte da disciplina} --- um erro de unidade no Silberschatz
\textit{et al.}, e três no Elmasri \& Navathe: um erro de definição (NL-SAS), uma imprecisão de
mecanismo (o controle de fluxo do FCoE) e um dado envelhecido (a plataforma \textit{Kinetic} da
Seagate) --- além de duas divergências em fontes de fabricante: a especificação do LTO-10, cuja taxa comprimida não fecha com a razão de
...
O presente relatório documenta um estudo comparativo das duas arquiteturas de armazenamento em
rede que sustentam a maior parte das instalações corporativas de Sistemas de Banco de Dados. A
primeira é o \textbf{NAS} (\textit{Network Attached Storage}), em que o equipamento de armazenamento
mantém um sistema de arquivos próprio e entrega \emph{arquivos} ao servidor pela mesma rede IP que a
empresa já usa. A segunda é a \textbf{SAN} (\textit{Storage Area Network}), em que o equipamento
entrega \emph{blocos} de disco por uma rede dedicada, e o servidor continua sendo o dono do sistema
de arquivos. A distância entre entregar arquivos e entregar blocos parece pequena, mas é dela que
decorrem todas as demais diferenças entre as duas soluções, e demonstrar isso é o que organiza o
trabalho.

Silberschatz, Korth e Sudarshan abrem o Capítulo 12 observando que o modelo lógico é o nível
correto para o usuário do banco pensar, já que ``o objetivo de um sistema de banco de dados é
simplificar e facilitar o acesso aos dados'' \cite{silberschatz2020}. Para quem implementa o
sistema, porém, essa simplificação se paga em complexidade: uma consulta SQL não muda de sintaxe
quando os arquivos de dados do banco saem de um disco local e passam a ser acessados pela rede, mas
o custo de cada acesso, a semântica de lock, o comportamento sob falha e o próprio conjunto de
configurações suportadas pelo fabricante do SGBD mudam completamente. A escolha da arquitetura de
armazenamento é um dos pontos em que essa distância entre o nível lógico e o nível físico fica mais
visível.

Elmasri e Navathe registram a causa econômica que levou o armazenamento para a rede: ``o custo
...
aquisição do armazenamento é ``apenas uma pequena fração, tipicamente apenas 10 a 15\%, do custo
geral de gerenciamento de armazenamento'' \cite{elmasri2018}. A migração para NAS e SAN respondeu,
portanto, a uma pressão de custo de operação antes de responder a uma pressão de desempenho, e o
objetivo desde o início foi desacoplar a capacidade de armazenamento do servidor específico que a
consome.

Além da comparação entre as duas arquiteturas, o trabalho cobre os protocolos de cada família com
suas normas de origem, discute as técnicas de \textit{Automated Storage Tiering} e de armazenamento
por objetos, e fecha com uma recomendação de uso organizada por nível da hierarquia de
armazenamento. O enunciado da tarefa fixa uma lista de tópicos obrigatórios, que adotamos como
\textit{checklist} literal e reproduzimos na Tabela~\ref{tab:checklist} junto com a localização de
cada item, de modo que um avaliador consiga conferir exigência por exigência sem precisar ler o
texto corrido.
...
Quanto ao método, adotamos uma regra única: todo número publicado declara sua origem, em uma de
cinco categorias. Uma especificação normativa é o valor fixado por uma norma, como uma RFC do IETF ou
um padrão T11/INCITS, e é o grau mais forte de proveniência porque não varia com o produto; um
\textit{datasheet} de fabricante vale para o modelo identificado, e não para a categoria; uma medição
de terceiro é o número obtido em publicação que descreva sua metodologia; uma derivação nossa é a
conta feita a partir das três anteriores, e nesse caso ela aparece explicitamente no texto. A quinta
categoria é a lacuna: quando não encontramos fonte, declaramos isso em vez de preencher a célula com
uma estimativa disfarçada de dado.

Verificamos também o rótulo de cada valor, porque o erro mais difícil de detectar é um número correto
que descreve outra coisa. Quatro armadilhas desse tipo apareceram na tarefa: taxa \textit{full-duplex}
contra taxa por direção, que diferem por um fator de dois e que a FCIA e a norma T11 publicam de
formas distintas (Seção~\ref{sec:fc}); bit contra byte, que produziu um erro no próprio Silberschatz
(Seção~\ref{sec:correcoes}); cliente contra servidor de um protocolo, distinção que altera em cinco
anos a data de descontinuação do AFP (Seção~\ref{sec:afp}); e taxa bruta de linha contra taxa útil.
Números voláteis, como preços e prazos de recuperação, recebem carimbo de data, já que um preço sem
data não informa nada.

As três fontes bibliográficas da disciplina cumprem papéis distintos no trabalho. Elmasri e Navathe
\cite{elmasri2018} são a fonte primária, porque a Seção 16.11 do Capítulo 16 é o único trecho dos
três livros que trata explicitamente de SAN, NAS, iSCSI, FCIP, FCoE, AST e armazenamento por
objetos, e dela extraímos a taxonomia e o vocabulário. Silberschatz, Korth e Sudarshan
\cite{silberschatz2020} fornecem a física do meio e a definição canônica dos níveis secundário e
terciário (Seção~\ref{sec:hierarquia}), que é o eixo pedido pelo enunciado na recomendação final;
vale registrar que a Seção 12.2 trata SAN e NAS em apenas dois parágrafos, e que a definição de
interface orientada a bloco está uma seção antes, na 12.1, no verbete sobre memória \textit{flash}.
Garcia-Molina, Ullman e Widom \cite{garciamolina2008} fornecem a formalização do modelo de custo de
E/S, isto é, a ideia de que o custo de uma consulta se mede em número de acessos a bloco e que a
latência do acesso domina o cálculo; é esse arcabouço que usamos na Seção~\ref{sec:engine} para
explicar por que a diferença entre NAS e SAN se manifesta em transações curtas e praticamente
desaparece em varreduras longas.
...
O enunciado pede a recomendação ``tanto em nível secundário (\textit{storage online}) como em nível
terciário (\textit{storage offline})''. Essa terminologia é a de Silberschatz \textit{et al.}, que a
define assim na Seção 12.1: ``as mídias de armazenamento mais rápidas, por exemplo \textit{cache} e
memória principal, são chamadas de armazenamento primário. As mídias no próximo nível da hierarquia,
por exemplo memória \textit{flash} e discos magnéticos, são chamadas de armazenamento secundário, ou
\textit{online storage}. As mídias no nível mais baixo da hierarquia, por exemplo fita magnética e
\textit{jukeboxes} de disco óptico, são chamadas de armazenamento terciário, ou \textit{offline
storage}'' \cite{silberschatz2020}.

Elmasri e Navathe usam a mesma divisão em três níveis e acrescentam o critério operacional que mais
importa aqui: ``dados em armazenamento secundário ou terciário não podem ser processados diretamente
pela CPU; primeiro precisam ser copiados para o armazenamento primário'' \cite{elmasri2018}. Disso
decorre que NAS e SAN são, ambos, tecnologias de nível secundário, já que os dois entregam
armazenamento \textit{online}, endereçável e disponível sem intervenção do operador. O armazenamento
por objetos, por sua vez, atravessa a fronteira: em suas classes de acesso imediato ele é
secundário, e em suas classes de arquivamento profundo, com prazos de recuperação medidos em horas,
ele funciona como terciário sem fita.

\subsection{O que o \textit{Database Engine} realmente pede ao armazenamento}
\label{sec:engine}

Antes de comparar arquiteturas, convém ser explícito sobre o que o núcleo de um SGBD exige do meio
em que grava. São cinco requisitos, aqui em ordem de rigidez.

O primeiro é o \textbf{acesso em unidades de bloco}. Silberschatz define que um SSD ``usa memória
\textit{flash} internamente para armazenar dados, mas provê uma interface similar à de um disco
magnético, permitindo que dados sejam armazenados ou recuperados em unidades de um bloco; tal
interface é chamada de interface orientada a bloco'' \cite{silberschatz2020}, e todo o modelo de
custo de Garcia-Molina \textit{et al.} \cite{garciamolina2008} é construído sobre essa unidade. Os
tamanhos típicos de bloco de banco de dados são 8~KiB no PostgreSQL e no Oracle, por padrão, e
16~KiB no InnoDB do MySQL.

O segundo é a \textbf{durabilidade sob comando explícito}. O \textit{commit} de uma transação só pode
retornar depois que o registro de \textit{log} correspondente estiver em mídia não volátil, o que se
implementa por uma chamada de sincronização como \texttt{fsync}, \texttt{fdatasync} ou uma escrita
com \texttt{O\_DSYNC}. É a latência dessa chamada que limita a taxa de \textit{commits} de uma carga
OLTP, e não a banda do canal.

O terceiro é a \textbf{atomicidade da escrita de página}. Se a energia cai no meio da gravação de
uma página de 16~KiB, o SGBD precisa detectar e reparar a página parcialmente escrita (\textit{torn
page}). O InnoDB resolve isso com o \textit{doublewrite buffer}, ``uma área de armazenamento onde o
InnoDB escreve as páginas descarregadas do \textit{buffer pool} antes de escrevê-las em suas posições
próprias nos arquivos de dados'', da qual consegue recuperar uma cópia boa se o sistema operacional,
o subsistema de armazenamento ou o processo \texttt{mysqld} saírem inesperadamente no meio da escrita
\cite{mysqldw}; o PostgreSQL resolve o mesmo problema com \texttt{full\_page\_writes} no WAL. Ambos
custam banda de escrita e podem ser desligados quando o dispositivo garante escrita atômica, mas
convém registrar o alcance real dessa concessão: o manual do MySQL não a estende a dispositivos com
\textit{atomic writes} em geral, e sim a um caso nomeado, o de dispositivos Fusion-io, e apenas sobre
NVMFS em Linux \cite{mysqldw}.

O quarto é a \textbf{semântica de lock previsível}. Em configurações de \textit{cluster} com
armazenamento compartilhado, como o Oracle RAC, vários nós escrevem no mesmo conjunto de blocos, e o
lock tem de permanecer correto sob falha de nó e sob partição de rede.

O quinto é o \textbf{comportamento determinado sob indisponibilidade transitória}. Se o caminho até o
armazenamento some por trinta segundos, o SGBD precisa saber se a operação de E/S vai bloquear,
retornar erro ou retornar sucesso falso, porque a diferença entre bloquear e errar decide se a
instância sobrevive ou aborta. É contra esses cinco requisitos que avaliamos NAS e SAN ao longo do
trabalho.

\subsection{Por que a rede entrou no caminho do dado}

O caminho histórico é DAS $\rightarrow$ NAS/SAN. No modelo DAS (\textit{Direct Attached Storage}), os
discos são cabeados ao servidor pelas interfaces SATA, SAS ou NVMe descritas por Silberschatz
\cite{silberschatz2020}. É a arranjo mais simples e de menor latência, mas tem dois defeitos
estruturais que Elmasri e Navathe apontam: a capacidade fica presa a um servidor específico, já que
``muitos usuários de sistemas RAID não conseguem usar a capacidade efetivamente porque ela precisa
estar ligada de maneira fixa a um ou mais servidores'', e o custo de gerenciamento cresce com o
número de ilhas \cite{elmasri2018}.

A resposta da indústria foi mover a fronteira de rede para dentro do caminho de E/S, e há duas
maneiras de fazer isso, que geram justamente as duas arquiteturas deste trabalho. A SAN move essa
fronteira \emph{abaixo} do sistema de arquivos: o servidor recebe blocos e continua sendo o dono do
sistema de arquivos. O NAS a move \emph{acima}: o servidor recebe arquivos, e o sistema de arquivos
passa a ser do dispositivo de armazenamento. Protocolo, cabo, latência, custo e modelo de falha são
todos consequência dessa escolha, e a Figura~\ref{fig:pilhas} mostra onde a fronteira cai em cada
caso.
...
Elmasri e Navathe definem o NAS a partir daquilo que ele deixa de fazer: os dispositivos NAS ``são,
de fato, servidores que não fornecem nenhum dos serviços comuns de servidor, mas simplesmente
...
Operacionalmente, o \textit{NAS head} possui discos ou SSDs organizados internamente, tipicamente em
RAID, e Elmasri e Navathe registram que ``tais dispositivos tipicamente suportam RAID níveis 0, 1 e
5'' \cite{elmasri2018}. Sobre esse armazenamento ele monta e opera um sistema de arquivos próprio,
que pode ser o WAFL da NetApp, o ZFS de muitos produtos ou ext4 e XFS em soluções abertas, e exporta
esse sistema de arquivos pela LAN por meio de um protocolo de compartilhamento, seja SMB/CIFS, NFS ou
AFP. O cliente, que aqui é o servidor de banco de dados, monta o compartilhamento e enxerga arquivos
e diretórios, emitindo operações de arquivo como \textsc{open}, \textsc{read}, \textsc{write},
\textsc{close} e \textsc{lock}, e não comandos de bloco.

O ponto decisivo está no sistema de arquivos próprio: quem executa a alocação de blocos, o
\textit{journaling} de metadados e o controle de concorrência de arquivo é o dispositivo NAS, ao
qual o servidor delega essas funções. Silberschatz resume a arquitetura em uma frase: ``NAS é muito
parecido com SAN, exceto que, em vez de o armazenamento em rede aparecer como um disco grande, ele
provê uma interface de sistema de arquivos usando protocolos de sistema de arquivos em rede como NFS
ou CIFS'' \cite{silberschatz2020}.

\subsection{Consequências diretas da semântica de arquivo}

Dessa delegação decorrem três consequências práticas. A primeira é que o compartilhamento entre
clientes é nativo: como o dispositivo é o dono do sistema de arquivos, é ele quem arbitra o acesso
concorrente, e vários servidores podem montar o mesmo compartilhamento sem \textit{software}
adicional. Em SAN, ao contrário, dois servidores que montem o mesmo LUN (\textit{Logical Unit Number}, o
volume de blocos que o equipamento de armazenamento apresenta ao servidor) com um sistema de
arquivos comum destroem os dados, a menos que se use um sistema de arquivos de \textit{cluster} como
GFS2, OCFS2 ou VMFS, ou um gerenciador de volume ciente de \textit{cluster}. A segunda é a independência
de sistema operacional do cliente, que Elmasri e Navathe registram ao afirmar que ``sistemas NAS
alegam maior independência de sistema operacional dos clientes'' \cite{elmasri2018}.

A terceira consequência é a mais problemática para banco de dados: a coerência de \textit{cache}. O
NFS não oferece coerência forte por padrão, e o manual do Linux é explícito ao dizer que ``o
protocolo NFS não foi projetado para suportar coerência de \textit{cache} verdadeira de sistema de
arquivos de \textit{cluster} sem algum tipo de serialização pela aplicação'', de modo que ``se
coerência absoluta de \textit{cache} entre clientes for necessária, as aplicações devem usar
lock de arquivo'', e que ``alternativamente, as aplicações podem abrir seus arquivos com a
\textit{flag} \texttt{O\_DIRECT} para desabilitar completamente o \textit{cache} de dados''
\cite{nfsman}. É por essa razão que os SGBDs suportados sobre NFS usam \texttt{O\_DIRECT} ou
substituem o cliente NFS do sistema operacional por um próprio, como se discute na
Seção~\ref{sec:dnfs}.
...
O SMB (\textit{Server Message Block}) é o protocolo de compartilhamento de arquivos nativo do mundo
Windows, e o CIFS (\textit{Common Internet File System}) não é um protocolo diferente: é o nome que a
Microsoft deu, em 1996, à família de dialetos hoje chamada SMB~1. A própria Microsoft posiciona a
relação dessa forma, já que a especificação aberta [MS-SMB2] define o SMB~2/3 como ``uma extensão do
protocolo \textit{Server Message Block} (SMB) original (conforme especificado em [MS-SMB] e
[MS-CIFS])'' \cite{mssmb2ovw}, isto é, o [MS-CIFS] documenta o SMB original e não um protocolo à
parte. Tratar ``SMB'' e ``CIFS'' como dois protocolos distintos é um erro comum, e a leitura correta
é que CIFS é SMB~1 e que tudo posterior a isso se chama SMB~2 ou SMB~3. O enunciado da tarefa usa a
grafia composta ``SMB/CIFS'', que é a convenção da indústria e a adotada aqui.

Trata-se de um protocolo de requisição-resposta orientado a sessão, transportado historicamente sobre
NetBIOS na porta 139 e, desde o Windows 2000, diretamente sobre TCP na porta 445. Suas operações são
de alto nível: abrir arquivo, ler ou travar um intervalo de \textit{bytes}, enumerar diretório e
consultar atributos. A Microsoft documenta a evolução dos dialetos na especificação aberta [MS-SMB2],
e a Tabela~\ref{tab:smb} reúne os marcos relevantes.
...
Quanto ao estado do SMB~1, a Microsoft documenta que ``a partir do \textit{Windows 10 Fall Creators
...
posteriores'' \cite{mssmb1}. Convém precisar o rótulo: o primeiro \textit{Windows Server} sem SMBv1
por padrão foi a versão 1709 do canal semianual, sendo o Server 2019 o primeiro do canal de suporte
prolongado. A consequência prática para NAS é que equipamentos antigos que só falam CIFS/SMB1 deixam
de ser montáveis por clientes modernos sem a reabilitação manual de um protocolo com
vulnerabilidades conhecidas, de modo que citar ``SMB/CIFS'' num projeto novo em 2026 significa, na
prática, especificar SMB~3.

A Microsoft suporta oficialmente colocar arquivos de banco de dados do SQL Server em compartilhamento
SMB: ``o SQL Server 2012 (11.x) e versões posteriores'' suportam armazenar bancos de dados de sistema
(\texttt{master}, \texttt{model}, \texttt{msdb}, \texttt{tempdb}) e de usuário em \textit{fileshare}
SMB, tanto em instalação isolada quanto em \textit{failover cluster} \cite{mssqlsmb}. As condições
que a documentação impõe expõem os pontos frágeis do NAS para banco de dados. Para cargas críticas,
``o compartilhamento de arquivos deve suportar \textit{SMB 3.0 transparent failover} (disponibilidade
contínua)'' \cite{mssqlsmb}, ou seja, o protocolo precisa sobreviver à troca de nó do servidor de
arquivos sem devolver erro ao SGBD. Além disso, a conta de serviço precisa de \texttt{FULL CONTROL}
no compartilhamento e no NTFS, o FILESTREAM não é suportado sobre SMB, e caminhos de
\textit{loopback}, compartilhamentos administrativos e unidades mapeadas também não são suportados,
restando apenas caminhos UNC.
...
O NFS (\textit{Network File System}) é o padrão aberto de compartilhamento de arquivos do mundo
UNIX/Linux, originalmente da Sun Microsystems em 1984 e hoje mantido pelo IETF. É o protocolo NAS que
efetivamente aparece em instalações de banco de dados, e a Tabela~\ref{tab:nfs} traz sua linha do
tempo com as normas correspondentes.
...
\textsc{commit} explícito, TCP. \textbf{Sem estado}: o lock fica \emph{fora} do protocolo, num
serviço separado (NLM, \textit{Network Lock Manager}), o que torna a recuperação após falha frágil.\strut} \\
\addlinespace
NFSv4.0 & \makecell[l]{2000\\(RFC 3010)} & \makecell[l]{RFC 7530\\(2015)} & \parbox[t]{7.3cm}{\raggedright \textbf{Com estado}. lock, montagem e
...
execução \textbf{exatamente uma vez} (\textit{exactly-once semantics}): vale para toda requisição
\textsc{compound}/\textsc{cb\_compound} precedida de uma operação \textsc{sequence}/%
\textsc{cb\_sequence} --- não é incondicional, mas a RFC é explícita em que a exigência ``vale
independentemente de a requisição ser enviada com \textit{reply caching} especificado''
\cite{rfc8881}: o \textit{reply cache} é mecanismo de implementação, não condição da garantia.
E \textbf{pNFS} (\textit{parallel NFS}), que separa o caminho de metadados do caminho de dados,
permitindo que o cliente leia dados diretamente de vários servidores em paralelo \cite{rfc8881}.\strut} \\
\addlinespace
...
Vale uma correção de rótulo sobre essa tabela: boa parte da literatura ainda cita a RFC 5661, de
2010, como a especificação do NFSv4.1, mas ela foi obsoletada pela RFC 8881, de 2020
\cite{rfc8881}. Ainda que o conteúdo técnico seja em grande parte o mesmo, citar a RFC 5661 hoje
aponta para um documento superado.

A maior limitação do NFS para banco de dados continua sendo a coerência de \textit{cache}. O
protocolo implementa a chamada \textit{close-to-open consistency}, em que o cliente verifica a
validade do \textit{cache} ao abrir o arquivo e descarrega as alterações pendentes ao fechá-lo
\cite{nfsman}, o que é adequado para arquivos de escritório e desastroso para um arquivo de dados de
banco que fica aberto por semanas sem nunca ser fechado. É por isso que todo SGBD suportado sobre NFS
ou usa \texttt{O\_DIRECT}, ou substitui o cliente NFS do sistema operacional pelo seu próprio.

\label{sec:dnfs}
A Oracle escolheu o segundo caminho. Segundo sua documentação, ``o cliente Direct NFS (dNFS) integra
a funcionalidade de cliente NFS diretamente no \textit{software} Oracle para otimizar o caminho de
E/S entre a Oracle e o servidor NFS'' \cite{oracledn}, ou seja, o SGBD implementa NFS dentro de si
mesmo, falando diretamente com o servidor NFS por \textit{sockets} próprios e ignorando o cliente
NFS do \textit{kernel}, o que lhe dá controle sobre profundidade de fila, número de conexões TCP e
semântica de \textit{cache}. A Oracle documenta que ``o cliente Direct NFS suporta os protocolos
NFSv3, NFSv4, NFSv4.1 e pNFS'', e que ``se o Oracle Database não conseguir se conectar a um servidor
NFS usando o cliente Direct NFS, então ele se conecta usando o cliente NFS do \textit{kernel} do
sistema operacional'' \cite{oracledn}.

A existência do dNFS é a evidência empírica mais direta da tese central deste trabalho, porque o
maior fornecedor de bancos de dados relacionais do mercado achou necessário reimplementar o protocolo
NAS dentro do próprio motor para poder usá-lo. Isso não desqualifica o NAS para banco de dados, mas
delimita a condição em que ele funciona: quando o motor participa da decisão, e não quando é tratado
como uma aplicação qualquer.
...
O AFP é o protocolo proprietário de compartilhamento de arquivos da Apple, originado no
\textit{AppleTalk Filing Protocol} de meados dos anos 1980. Sua característica distintiva era
preservar corretamente as particularidades do sistema de arquivos do Macintosh, isto é,
\textit{resource forks}, tipo e criador de arquivo, nomes longos com acentuação e metadados que o
SMB1 da época mutilava. Por muitos anos foi o único jeito de fazer um NAS servir Macs sem corromper
arquivos, e era o protocolo obrigatório para \textit{backups} do Time Machine em rede.

Trata-se de um protocolo de sessão, com estado, transportado historicamente sobre AppleTalk
(ASP/ATP) e, desde o AFP 2.1, também sobre TCP na porta 548 por meio do DSI (\textit{Data Stream
Interface}). A documentação da Apple é literal quanto a isso: ``\textit{TCP can be used as the
transport protocol for AFP version 2.1 and later}'' \cite{appleafptcp}. O que o AFP 3.0, de 2001,
muda não é a existência do transporte TCP, e sim sua exclusividade, já que a partir dele o AppleTalk
fica restrito à descoberta de serviço.

Três características determinam seu comportamento. A sessão é com estado: o cliente executa
\textsc{FPLogin}, abre um \textit{volume} com \textsc{FPOpenVol} e daí em diante opera sobre
identificadores de sessão, de modo que uma falha de rede a derruba, problema que o AFP 3.x mitigou
com reconexão. Os comandos são bifurcados por \textit{fork}, com \textsc{FPOpenFork} e
\textsc{FPRead}/\textsc{FPWrite} operando sobre o \textit{data fork} ou o \textit{resource fork} de
um arquivo do Macintosh, e essa é a razão técnica original de o protocolo existir, já que SMB1 e NFS
só emulavam isso com truques como arquivos \texttt{.\_nome} e \textit{AppleDouble}, ao custo de
corrupção silenciosa de metadados; pela mesma lógica, os metadados do Finder e os nomes longos em
UTF-8 com decomposição canônica trafegam como atributos de primeira classe. Por fim, o lock por
intervalo de \textit{bytes} existe, via \textsc{FPByteRangeLock}, mas com semântica de sessão: morre
com ela, e não há equivalente às \textit{leases} com prazo do NFSv4 nem aos \textit{oplocks} do
SMB~2/3.

O que o AFP nunca teve é a razão de não servir a banco de dados: nenhum caminho de dados com RDMA,
nenhum equivalente a \textit{multichannel}, nenhuma disponibilidade contínua com \textit{failover}
transparente, e um lock amarrado à sessão. Comparado ao SMB~3 (Seção~\ref{sec:smb}) e ao
NFSv4.1 (Seção~\ref{sec:nfs}), ele parou no conjunto de recursos de um protocolo de compartilhamento
de arquivos de escritório.

Quanto ao estado atual, e é aqui que a maior parte do material disponível erra, o AFP está sendo
removido do macOS em duas etapas separadas por cerca de cinco anos: o servidor, isto é, a capacidade
de um Mac compartilhar pastas por AFP, saiu no macOS 11 Big Sur, em 2020; o cliente, a capacidade de
montar um compartilhamento AFP de um NAS, foi depreciado no macOS 15.5 Sequoia, em maio de 2025, com
a Apple registrando que ``o cliente do \textit{Apple Filing Protocol} (AFP) está depreciado e será
removido em uma versão futura do macOS'' \cite{appleafp}. Para o macOS 27, a Apple informa
oficialmente que o Time Machine deixa de suportar destinos AFP, incluindo Time Capsules e AirPort
Disks \cite{appleafptm}.
...
A relevância do AFP para SBD é nenhuma, e esse é justamente o achado. Pesquisamos as matrizes de
suporte publicadas por Oracle, Microsoft, PostgreSQL e MySQL e não localizamos AFP em nenhuma delas,
de modo que o que afirmamos é que nenhum dos quatro SGBDs de maior participação o lista como
configuração suportada, e não um negativo universal sobre todo o mercado. O motivo técnico está nas
características acima: sem lock independente de sessão, sem RDMA e sem disponibilidade
contínua, o AFP não atende aos requisitos quatro e cinco da Seção~\ref{sec:engine}. Para um NAS que
precise servir clientes Apple, a recomendação em 2026 é SMB~3, protocolo primário do macOS desde o
OS~X 10.9 Mavericks, de 2013. Se o AFP aparece na lista do enunciado, é como item de completude
histórica, e é mais honesto dizer isso do que fabricar uma aplicação de banco de dados para ele.

\subsection{Vantagens e desvantagens do NAS}
\label{sec:vantnas}

\correcao{Declaração de lacuna: os itens de custo desta tabela não têm número}{%
As linhas de custo da Tabela~\ref{tab:vantnas} e das tabelas comparativas seguintes ---
``menor custo por TB útil'', ``custo relativo menor/maior'' --- são \textbf{afirmações estruturais,
não medições}. Elas se sustentam em um fato verificável (o NAS dispensa a HBA, que é a placa adaptadora de
barramento instalada no servidor para falar com a rede de armazenamento, e dispensa também o
\textit{switch} FC, os transceptores e o cabeamento dedicados que a SAN exige) e em outro que \emph{não} conseguimos
...
unificado que permitisse uma comparação honesta e datada, porque preço de canal depende de volume e
de contrato, e reproduzi-lo sem data violaria a regra de que número volátil sem data não é dado.
...
\texttt{O\_DIRECT}, lock explícito ou cliente dedicado \cite{nfsman}. \\
Provisionamento simples: criar um compartilhamento e conceder permissão, sem zoneamento nem
mapeamento de LUN. & Camada extra de sistema de arquivos no caminho: metadados, lock e
...
de uma leitura aleatória em SSD --- que Silberschatz situa em ``20 a 100 microssegundos''
\cite{silberschatz2020}, para SSDs em geral, sem particularizar NVMe --- a rede FC responde por
\textbf{menos de 5\%} do tempo total do acesso.
...
zoneamento. O FCIP carrega quadros FC através da WAN IP. Sem isolamento por roteamento FC,
a extensão pode unir as \textit{fabrics} e propagar eventos de reconfiguração entre sítios.
Por isso os produtos oferecem \textit{FC routing} / \textit{IVR} para isolar as \textit{fabrics}
mantendo o acesso entre elas.}

\textbf{Aplicação em recuperação de desastre.} Replicação síncrona ou assíncrona entre sítios para
...
O FCoE (\textit{Fibre Channel over Ethernet}) encapsula quadros FC inteiros diretamente em quadros
Ethernet, sem TCP e sem IP. Foi padronizado pelo T11 em FC-BB-5, publicado como ANSI/INCITS 462-2010
\cite{fcbb5}, e a versão seguinte, FC-BB-6, acrescentou o modo VN2VN, com comunicação direta entre
nós sem \textit{switch} FCF.

Elmasri e Navathe tratam o protocolo em um parágrafo que vale reproduzir por inteiro, porque
recortá-lo mudaria o julgamento. O livro escreve que ``a ideia mais recente a entrar na corrida do
armazenamento IP corporativo é o \textit{Fibre Channel over Ethernet} (FCoE), que pode ser pensado
como iSCSI sem o IP. Ele usa muitos elementos de SCSI e FC (assim como o iSCSI), mas não inclui
componentes TCP/IP. [\ldots] Ele tira proveito de uma tecnologia Ethernet confiável que usa
\textit{buffering} e controle de fluxo fim-a-fim para evitar pacotes descartados''
\cite{elmasri2018}. A frase de abertura é analogia didática, e não definição, e o próprio parágrafo
já a qualifica ao dizer que o FCoE não inclui TCP/IP e que depende de Ethernet confiável com controle
de fluxo.

Restam, contudo, duas imprecisões reais e específicas. A primeira é que o controle de fluxo do FCoE
não é fim-a-fim, e sim enlace a enlace. O mecanismo é o IEEE 802.1Qbb, \textit{Priority-based Flow
Control} \cite{ieee8021qbb}: o quadro \textsc{pause} atua entre dois vizinhos adjacentes, por
prioridade, e cada salto exerce a pressão sobre o salto anterior, sem realimentação entre origem e
destino como há no TCP. A distinção tem consequência prática, porque é ela que produz o
\textit{congestion spreading} característico de redes FCoE mal dimensionadas, em que um alvo lento
propaga pausas para trás e degrada tráfego não relacionado que compartilhe o caminho.

A segunda imprecisão é a de maior consequência, e é a que a analogia apaga: o FCoE não é roteável em
camada 3, enquanto o iSCSI é. O FCoE usa um \textit{EtherType} próprio, o 0x8906 para o quadro de
dados e o 0x8914 para o FIP, que é o protocolo de inicialização, encapsulado diretamente em quadro
Ethernet; como não há cabeçalho IP, não há o que rotear, e o FCoE vive dentro de um domínio de
camada 2. O iSCSI, por rodar sobre TCP/IP, atravessa LAN, WAN e a Internet, vantagem que o próprio
livro destaca ao tratar dele \cite{elmasri2018}. Um projetista que leia ``FCoE é iSCSI sem o IP'' e
conclua que pode usá-lo entre dois \textit{data centers}, como faria com iSCSI, vai descobrir que não
pode: para atravessar a WAN, os caminhos são FCIP, que é um túnel entre \textit{fabrics} FC, ou o
próprio iSCSI. Registramos a categoria dessa análise: é juízo técnico do grupo apoiado em norma, o
FC-BB-5 e o IEEE 802.1Qbb, e não citação.

Quanto ao lugar que o FCoE ocupa, sua promessa era a convergência, isto é, um único par de
adaptadores (CNAs) e um único cabeamento carregando LAN e SAN, reduzindo adaptadores, cabos, portas
e consumo. Elmasri e Navathe registram a adoção ao afirmar que ``o FCoE foi produtizado com sucesso
pela CISCO (denominado \textit{Data Center Ethernet}) e pela Brocade'' \cite{elmasri2018}. Na
prática, porém, a convergência prevaleceu dentro do rack, em \textit{blade chassis} e sistemas
convergentes, onde o domínio de camada 2 é pequeno e controlado, e não substituiu o FC no núcleo da
SAN nem deslocou o iSCSI nas instalações menores, de modo que hoje o FCoE ocupa um nicho estreito
entre dois vizinhos mais simples.

\subsection{Dois mecanismos que a SAN pressupõe: \textit{multipath} e RAID}
\label{sec:multipath}

O quinto requisito da Seção~\ref{sec:engine}, o comportamento determinado sob indisponibilidade
transitória, não é atendido pelo protocolo sozinho. Quem o atende são dois mecanismos que ficam fora
da discussão de protocolo e dentro da de arquitetura.

O primeiro é o \textit{multipath}. Montar duas \textit{fabrics} independentes
(Seção~\ref{sec:topologias}) só produz disponibilidade se houver, no servidor, uma camada que
reconheça que os dois caminhos levam ao mesmo LUN e escolha entre eles, papel do DM-Multipath no
Linux e do MPIO no Windows. Em \textit{arrays} com controladoras assimétricas, o padrão ALUA
(\textit{Asymmetric Logical Unit Access}) permite ao alvo informar quais caminhos são otimizados e
quais são apenas disponíveis, para que o \textit{host} não force o \textit{array} a atravessar o
\textit{interconnect} interno entre controladoras. A consequência direta para o SGBD é que o que
decide se uma falha de caminho é invisível ou fatal é o tempo de detecção e de reprova, configurado
em \texttt{no\_path\_retry} e \texttt{fast\_io\_fail\_tmo}: configurá-lo como ``enfileirar
indefinidamente'' congela a instância, e como ``falhar imediatamente'' aborta transações. É decisão
de projeto, e não padrão a aceitar, sendo essa a resposta concreta ao quinto requisito.

O segundo mecanismo é o RAID, e o que importa aqui é a penalidade de escrita aplicada ao \textit{log}.
Silberschatz dedica a Seção 12.5 ao tema e é explícito quanto ao custo do nível 5: ``o RAID nível 5
tem uma sobrecarga significativa para escritas aleatórias, já que uma única escrita aleatória de
bloco requer 2 leituras de bloco (para obter os valores antigos do bloco e do bloco de paridade) e 2
escritas de bloco'' \cite{silberschatz2020}. No mesmo trecho ele conclui que ``o RAID nível 1 é
popular para aplicações como o armazenamento de arquivos de \textit{log} num sistema de banco de
dados, já que oferece o melhor desempenho de escrita'' \cite{silberschatz2020}.
...
$4\times$, e é justamente a taxa de \textit{commits} que ela limita. Daí a regra prática que o
próprio livro enuncia: \textit{log} em RAID 1 ou 10, dados em RAID 5 ou 6. É também a razão pela qual
a política de AST para o volume de \textit{redo} deve ser fixada, e não deixada ao critério
estatístico do \textit{array} (Seção~\ref{sec:ast}).}
...
Latência de \emph{comutação local} pequena diante da latência da mídia ($<$5\% num acesso NVMe;
derivação e ressalvas na Seção~\ref{sec:topologias} --- a conta não cobre HBA, ISLs, filas nem
controladora). & \textit{Boot} e recuperação dependem da \textit{fabric}: um erro de
...
Boa parte da confusão entre os cinco protocolos, e quase toda a confusão entre iSCSI, FCIP e
FCoE, desaparece quando se vê o que cada um coloca dentro de o quê. A Figura~\ref{fig:encaps}
mostra as cinco pilhas lado a lado, antes da grade comparativa.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig/encapsulamento.pdf}
\caption{O que trafega na rede em cada arquitetura. A figura torna imediato que iSCSI e FCoE não
são variantes um do outro, já que o iSCSI carrega comandos SCSI sobre TCP/IP enquanto o FCoE carrega
o quadro FC inteiro sobre Ethernet; que FCIP e FCoE carregam a mesma coisa, o quadro FC, diferindo
apenas em sobre o quê, e é daí que um é roteável e o outro não; e que o NAS é o único em que a
unidade não é o bloco, porque no topo da pilha há operações de arquivo e não comandos SCSI, que é a
diferença discutida na Seção~\ref{sec:nas}.}
\label{fig:encaps}
\end{figure}

A Tabela~\ref{tab:protocolos} reúne em uma única grade todos os protocolos listados pelo
enunciado, cada um como uma linha, com sua norma de origem e as propriedades que decidem sua
...
Sim & RFC 8881 especifica a v4.1; pNFS separa metadados de dados \cite{rfc8881} & \textbf{Suportado}
conforme o SGBD e a configuração homologada; considerar a coerência
...
de arquivo & TCP porta 548 & Sim & \textbf{Legado}: servidor removido no macOS 11 (2020);
...
\textbf{Legado, sem recomendação para novas implantações.} Possui lock por intervalo de bytes, mas não identificamos configuração de SGBD homologada neste estudo \\
...
latência dependente dos saltos, carga e controladora. Utilizado por Oracle ASM e \textit{clusters} com
...
FSPF & Não & Topologia predominante em SAN FC; FC-AL legado &
Banda por porta e zoneamento; duas \textit{fabrics}
independentes (SAN A / SAN B) são recomendadas para alta disponibilidade \\
...
\textbf{FCIP} & SAN & IETF \textbf{RFC 3821} (2004), \emph{vigente}, atualizada pela RFC 7146 (2014)
quanto aos requisitos de IPsec v3 \cite{rfc3821,rfc7146} & \textbf{Quadros FC inteiros}
em TCP/IP & TCP/IP sobre WAN & Sim & Extensão de FC por IP & Túnel entre sítios, usado em replicação e \textit{backup}. A distância impõe
...
\textbf{Não} --- sem cabeçalho IP no encapsulamento \cite{fcbb5}, não há o que rotear em L3
(análise nossa, Seção~\ref{sec:fcoe}) & Nicho: convergência dentro do rack (\textit{blade}, sistemas
convergentes) & Viável em infraestrutura Ethernet compatível com DCB; não é roteado diretamente por IP \\
...
Filas paralelas e conjunto de comandos NVMe; depende do transporte e da implementação \\

\end{longtable}}
\end{landscape}
\restoregeometry

\subsection{NAS $\times$ SAN: as dimensões que decidem a escolha}
...
Rede & Ethernet/IP, compartilhada ou segregada & FC ou Ethernet/IP; isolamento depende do projeto \\
...
Lock & No protocolo (NLM no NFSv3; integrado a partir do NFSv4; \textit{oplocks}/\textit{leases}
no SMB) & No servidor; a SAN não sabe o que é um arquivo \\
Coerência de \textit{cache} & Depende do protocolo e do cliente; NFS usa \textit{close-to-open} \cite{nfsman} & Coordenada pelos servidores ao compartilhar blocos \\
Custo relativo & Depende de capacidade, rede e suporte & Depende de capacidade, rede, HBA e suporte \\
Distância & Rede IP; latência limita a aplicação & Depende da óptica e do transporte; FCIP não elimina a latência \\
Caso de uso típico em SBD & Arquivos de \textit{backup}, análise e bancos em configurações NFS/SMB homologadas & OLTP crítico, \textit{clusters} com
armazenamento compartilhado, cargas com requisito de latência de \textit{commit} \\
\bottomrule
\end{tabular}}
\end{table}

Convém fechar a comparação com uma ressalva que os próprios livros já anunciavam. Silberschatz
registra que ``sistemas SAN e NAS modernos suportam o uso de uma combinação de discos magnéticos e
SSDs, e podem ser configurados para usar os SSDs como \textit{cache} para dados que residem em
discos magnéticos'' \cite{silberschatz2020}. Na prática de 2026, quase todo \textit{array}
corporativo é unificado: o mesmo equipamento apresenta LUNs por FC ou iSCSI e compartilhamentos por
NFS e SMB, sobre o mesmo \textit{pool} de mídia. A pergunta ``comprar NAS ou SAN?'' virou, em grande
medida, ``qual protocolo apresentar para cada carga?'', que é uma pergunta melhor porque admite
respostas diferentes para o \textit{tablespace} e para a área de \textit{dump}.
...
O AST existe porque a hierarquia de armazenamento de Silberschatz (Seção~\ref{sec:hierarquia})
ordena os meios por preço por byte, e a distribuição de acesso a dados é bastante enviesada: em
praticamente qualquer banco de dados corporativo, uma fração pequena dos blocos concentra a maioria
dos acessos. Comprar \textit{flash} para o banco inteiro significa, portanto, pagar pelo pior caso
em toda a capacidade instalada.
...
publica os parâmetros exatos \cite{dellfast}, o que situa a proveniência desta seção na categoria de
documentação de fabricante com produto identificado.
...
O mecanismo, portanto, combina contadores de acesso por fatia, uma métrica agregada de
temperatura e uma classificação horária, e só então move fisicamente as fatias entre \textit{tiers},
durante uma janela em que o impacto sobre a carga de produção seja aceitável.

\subsection{AST e \textit{buffer pool}}
\label{sec:asttensao}

A comparação a seguir distingue a migração de dados no array do cache de páginas do SGBD; granularidade e janela de observação podem levar a decisões diferentes.

\destaque{Buffer pool e AST: mecanismos complementares}{%
...
\cite{elmasri2018}. O \textit{buffer pool} mantém cópias de páginas em RAM; AST migra dados entre camadas de armazenamento. As diferenças relevantes são:
...
\item \textbf{Cache e AST atendem objetivos diferentes.} Cache mantém uma cópia na camada
rápida; AST migra a localização dos dados. Ambos têm custo de movimentação, e a reação
depende da implementação. Silberschatz descreve SSDs como cache de discos magnéticos
\cite{silberschatz2020}. Não concluímos que um mecanismo sempre substitui o outro em OLTP.
\end{enumerate}
...
\item \textbf{Abre mão do POSIX} $\rightarrow$ sem \texttt{fsync}, sem lock em intervalo de
...
objeto'', valendo também para operações de listagem \cite{awss3faq}. \textbf{Essa garantia refere-se às operações de objetos e listagem, não a transações entre chaves} --- e
essa era uma das afirmações que a nossa própria primeira versão continha (ver
Seção~\ref{sec:erros}).}

\subsection{Limites para motores OLTP que exigem acesso a blocos}

Uma API de objetos não substitui diretamente o dispositivo de blocos ou sistema de arquivos
esperado por um motor tradicional. A ressalva de Elmasri e Navathe \cite{elmasri2018} deve ser
lida nesse contexto, não como impossibilidade de construir um SGBD sobre objetos.
\begin{itemize}[leftmargin=0.7cm,itemsep=2pt]
\item A publicação por chave não oferece a mesma interface de atualização de páginas.
\item Latência, localização e tamanho das requisições influenciam o desempenho \cite{silberschatz2020}.
\item Não há \texttt{fsync} de arquivo nem transação nativa entre chaves: o motor deve implementar
ordenação do log, concorrência e recuperação. Consistência forte por objeto não resolve isso sozinha.
\end{itemize}

\subsection{Aplicações adequadas}

\begin{itemize}[leftmargin=0.7cm,itemsep=2pt]
\item \textbf{Camada de armazenamento de \textit{data warehouses} em nuvem.} Objetos podem guardar dados colunares com uma camada de metadados transacional. Varreduras de grandes volumes amortizam o custo das requisições; latência continua relevante.
...
\smallskip
\textbf{(1) Fita e SAN.} Drives podem ser conectados por FC à SAN ou por SAS
diretamente a um servidor. Se backup e produção compartilham enlaces, disputam capacidade;
zoneamento controla acesso, não reserva banda. Dimensionar a rede ou separar os caminhos.

\smallskip
\textbf{(2) NAS como estágio em disco.} Um compartilhamento NAS pode receber os arquivos
de backup antes da cópia para fita (\textit{disk-to-disk-to-tape}); não é obrigatório:
o repositório também pode usar disco local ou LUN de SAN.

\smallskip
\textbf{(3) NDMP como opção.} O \textit{Network Data Management Protocol} separa controle
e dados no backup de equipamentos compatíveis. Não é requisito universal de NAS;
a escolha depende do suporte do equipamento e do software de backup.}
...
acesso (derivação da Seção~\ref{sec:topologias}). \textbf{Alcance da afirmação:} a conta mede a
\emph{comutação local}, não o caminho fim a fim, que ainda soma HBA, ISLs, filas e controladora;
o que ela mostra é que o argumento de velocidade deixou de ser o eixo da escolha, não que a rede
tenha deixado de importar. O que sobra como
diferença entre as arquiteturas é semântico: quem garante a atomicidade da página, quem arbitra o
lock, o que acontece quando o caminho oscila. É por isso que a Microsoft, ao suportar SQL
...
{\footnotesize
\begin{longtable}{@{}p{3.7cm}p{4.3cm}p{6.3cm}@{}}
...
Post-Mortem. &
Decidiu priorizar o escopo do enunciado em vez de repetir o porte da tarefa
...
Seções \ref{sec:ast} (AST) e \ref{sec:objeto} (\textit{object storage}); apoio à conferência
complementar do Post-Mortem em 06/09/2026, executada pelo Codex. &
Decidiu adotar a \textbf{tensão entre AST e \textit{buffer manager}} como eixo analítico da seção de
\textit{tiering} (Seção~\ref{sec:asttensao}). Razão: relacionar a migração no array com as decisões de cache do SGBD. Decidiu também
reverificar se a objeção clássica de consistência eventual do S3 ainda valia --- não valia, e a
verificação corrigiu um erro da IA. Na conferência complementar, decidiu tratar o plano de correções
como fotografia de uma versão anterior e verificar cada apontamento contra os artefatos atuais. \\
\addlinespace
...
\destaque{Conferência complementar do Post-Mortem solicitada por Raphael --- 06/09/2026}{%
A pedido de Raphael Henrique da Silva Pereira, o Codex comparou a auditoria complementar com o relatório, os slides e o
verificador atuais. Confirmou que os principais apontamentos já haviam sido incorporados: ressalvas
de escopo para latência e durabilidade, recomendações condicionadas por suporte e SLA, distinções de
AST e leitura efetiva do PPTX por OOXML quando necessário. A atividade não alterou a contagem das 33
correções técnicas, porque registrou o fechamento de achados históricos, e não uma nova rodada.}

\destaque{Responsável principal pelo Post-Mortem}{Guilherme En Shih Hu permaneceu responsável pela
redação do Post-Mortem e pela condução das três rodadas adversariais. A conferência solicitada por
Raphael foi apoio complementar e não altera essa atribuição.}
...
Lista completa das \textbf{42 correções} aplicadas em três rodadas. A coluna ``conferido na fonte
...
\cite{appleafptm}. & Grupo (pesquisa) & Gabriel Schmitz \\
2 & ``A FCIA publica \textit{full-duplex}'', apresentado como citação. & Nenhum documento da FCIA
diz isso. Reclassificado como derivação nossa. & Grupo (pesquisa) & Bernardo Brandão \\
3 & \textit{Speedmap} v21 (2016) apresentado como vigente. & Data explicitada; ressalva de revisão
acrescentada. \emph{Correção incompleta --- ver \#19.} & Grupo (pesquisa) & Bernardo Brandão \\
4 & ``Codificação 64b/66b a partir de 16GFC.'' & 64b/66b só no 16GFC; de 32GFC em diante é
256b/257b com RS-FEC. & Grupo (pesquisa) & Bernardo Brandão \\
5 & ``CIFS $\equiv$ SMB 1.0.'' & A Microsoft diz que o CIFS é um \emph{dialeto} do SMB. & Grupo (pesquisa) &
Gabriel Schmitz \\
...
\subsubsection*{Terceira rodada (9 correções) --- revisão de proveniência das Seções 1--6}

Rodada conduzida sobre as Seções \ref{sec:nas} a \ref{sec:ast} com um critério único: \emph{para
cada número e cada frase entre aspas, abrir o documento primário e conferir se a fonte diz aquilo}.
Foi a rodada que encontrou o defeito mais constrangedor deste trabalho --- uma frase apresentada
entre aspas como texto de norma ANSI, sendo texto de enciclopédia colaborativa.

{\scriptsize \setlength{\tabcolsep}{3pt}
\begin{longtable}{@{}p{0.35cm}p{4.6cm}p{5.5cm}p{2.0cm}p{2.0cm}@{}}
\toprule
\textbf{\#} & \textbf{Defeito da versão 2} & \textbf{Correção aplicada} & \textbf{Gravidade} &
\textbf{Conferido na fonte por} \\
\midrule
\endfirsthead
\toprule
...
34 & \textbf{Tabela \ref{tab:protocolos} atribuía à norma FC-BB-5, entre aspas, a frase ``não é
roteável na camada IP e não funcionará através de redes IP roteadas como a Internet''.} O texto é o
da Wikipédia, não o da ANSI/INCITS 462-2010 --- norma paga, que não acessamos. & Aspas removidas. A
afirmação passa a ser análise nossa, apoiada na norma apenas quanto ao encapsulamento (sem cabeçalho
IP \\Rightarrow$ nada a rotear), com remissão à Seção~\ref{sec:fcoe}. & \textbf{Gravíssima} ---
proveniência fabricada num trabalho cuja tese é a proveniência & Guilherme En Shih \\

35 & \textbf{O Resumo declarava ``cinco divergências no material-fonte''} e atribuía duas ao
Silberschatz; a Tabela~\ref{tab:correcoes} lista quatro (1 Silberschatz \+$ 3 Elmasri). Declarava
ainda ``um dado desatualizado por um fator de dezesseis'', expressão que \textbf{não aparece em
nenhum outro ponto do relatório}. & Recontado para quatro, com a atribuição correta por livro.
Alegação sem lastro removida. & \textbf{Grave} --- é a reincidência exata do \#33 & Vivian Maria \\

36 & ``Historicamente sobre AppleTalk e, \textbf{desde o AFP 3.0}, sobre TCP na porta 548.'' & A
documentação da Apple diz ``\textit{TCP can be used [\ldots] for AFP version 2.1 and later}''
\cite{appleafptcp}. O AFP 3.0 não introduz o TCP: torna-o exclusivo, relegando o AppleTalk à
descoberta. & Média & Gabriel Schmitz \\

37 & ``A garantia de \textit{exactly-once} do NFSv4.1 vem do par sessão \+$ \textit{reply cache}.''
& A RFC 8881 §2.10.6 diz o oposto: a exigência ``vale independentemente de a requisição ser enviada
com \textit{reply caching} especificado'' \cite{rfc8881}. A condição é o \textsc{sequence} inicial,
não o cache. & Média & Enzo de Carvalho \\

38 & \textbf{A correção \#27 estava declarada no Post-Mortem, mas nunca chegou ao corpo do texto:} a
Seção~\ref{sec:engine} seguia dizendo ``dispositivos com suporte a \textit{atomic writes}'' em geral.
& Aplicada de fato: o manual restringe a Fusion-io sobre NVMFS em Linux \cite{mysqldw}. &
\textbf{Grave} --- declarar correção não aplicada é pior que não corrigir & Raphael Henrique \\

39 & Tabela~\ref{tab:vantsan} dizia ``latência de rede desprezível'' e a Conclusão, ``a rede saiu do
caminho crítico'' --- ambas além do que a própria Seção~\ref{sec:topologias} autoriza, que já
ressalva não medir HBA, ISLs, filas nem controladora. & Reduzidas ao alcance da conta:
\emph{comutação local}. & \textbf{Grave} --- família (d), contradição interna & Bernardo Brandão \\

40 & ``A documentação da própria Microsoft é explícita ao afirmar que o CIFS é um dialeto do SMB'',
sem citação. & Citado o [MS-SMB2] §1.3, que define SMB~2/3 como extensão do SMB original
``especificado em [MS-SMB] e [MS-CIFS]'' \cite{mssmb2ovw}. & Baixa & Vivian Maria \\

41 & FCIP citado como ``RFC 3821 (2004)'', sem registrar seu estado atual --- assimetria em relação
ao rigor aplicado ao par RFC 5661/8881 (\#16). & Registrado: vigente, \emph{não} obsoletada, mas
atualizada pela RFC 7146 (2014) quanto a IPsec v3 \cite{rfc7146}. & Baixa & Gabriel Schmitz \\

42 & FCoE descrito com um único \textit{EtherType} (0x8906), embora o texto mencione o FIP. &
Acrescentado o 0x8914, do FIP. & Baixa & Enzo de Carvalho \\
\end{longtable}}

\destaque{Padrão dos erros --- a lição que levamos}{%
Das 42 correções, \textbf{uma única} --- a \#34 --- é do tipo que se costuma chamar de
``alucinação'': uma frase de enciclopédia colaborativa apresentada entre aspas como texto de norma
ANSI. As outras 41 não são invenção. \textbf{Cinco famílias}, agora que temos três rodadas para
comparar:

\smallskip
\textbf{(a) Rótulo errado sobre número certo} --- \#2, \#4, \#6, \#7, \#8, \#10, \#13, \#14, \#22,
\#23, \#25, \#26, \#27, \#31, \#36, \#37, \#42: \textbf{dezessete casos}, a família mais numerosa e a
que sobreviveu às duas primeiras rodadas. O valor existe na fonte, mas descreve outra coisa. \textbf{Antídoto:} perguntar
sempre ``este número mede exatamente o quê, em que condições?''.

\smallskip
\textbf{(b) Desatualização} --- \#1, \#3, \#15, \#16, \#17, \#19, \#41: sete casos. \textbf{Antídoto:}
carimbar data e reverificar qual norma está vigente.

\smallskip
\textbf{(c) Excesso de confiança na formulação} --- \#5, \#9, \#11, \#12, \#24, \#28, \#32, \#33,
\#35, \#38, \#40: \textbf{onze casos}. ``Não há fonte'' vira afirmação categórica; conta citada não é
refeita; correção declarada no Post-Mortem não é aplicada ao corpo do texto (\#38).

\smallskip
\textbf{(d) Recorte que favorece o próprio argumento} --- \#20, \#21, \#29, \#39: \textbf{quatro
casos, e estão entre os mais graves.} Não são erros de fato: são erros de \emph{enquadramento}. A IA citou o
...
pergunta ``o que foi deixado de fora?''.

\smallskip
\textbf{(e) Proveniência fabricada} --- \#34: \textbf{um caso, e é o pior de todos.} Uma frase
correta \emph{de fato}, com a fonte \emph{errada} --- texto de enciclopédia colaborativa entre aspas,
creditado a uma norma ANSI paga que não tínhamos como consultar. Nenhuma checagem de fato pega isso:
o fato confere. Só pega quem vai ao documento citado e procura a frase lá dentro. \textbf{Antídoto:}
não usar aspas sem ter o documento aberto; quando a norma for paga e inacessível, dizer isso e
converter a afirmação em análise própria. É o achado metodológico mais importante deste trabalho,
e a razão de existir a terceira rodada.}
...
registra agora, para cada uma das 42 correções, quem foi ao documento primário confirmá-la antes de
