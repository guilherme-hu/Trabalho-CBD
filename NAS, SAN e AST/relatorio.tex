...
\item \textbf{Silberschatz, Korth \& Sudarshan, Cap.\ 12} \cite{silberschatz2020} --- fornece a
      ...
      Seção 12.2 trata SAN e NAS em dois parágrafos; a definição de \emph{interface orientada a
      bloco} está uma seção antes, na 12.1, no verbete sobre memória \textit{flash}.
...
atômica. \textbf{Precisão de alcance:} o manual do MySQL não faz essa concessão a ``dispositivos com
\textit{atomic writes}'' em geral; ele a restringe a um caso nomeado --- o \textit{doublewrite} é
desabilitado automaticamente quando reside em dispositivo Fusion-io com suporte a escrita atômica,
e apenas sobre NVMFS em Linux \cite{mysqldw}.
...
SMB~1. A própria Microsoft posiciona assim: a especificação aberta [MS-SMB2] define o SMB~2/3 como
``uma extensão do protocolo \textit{Server Message Block} (SMB) original (conforme especificado em
[MS-SMB] e [MS-CIFS])'' \cite{mssmb2ovw} --- isto é, [MS-CIFS] documenta o SMB original, não um
protocolo à parte. Tratar ``SMB'' e ``CIFS'' como dois protocolos distintos é um erro comum; a
...
sobre AppleTalk (ASP/ATP) e, \textbf{desde o AFP 2.1}, também sobre TCP na \textbf{porta 548} por
meio do \textbf{DSI} (\textit{Data Stream Interface}), a camada que enquadra requisições e respostas
e multiplexa comandos numa conexão TCP. A documentação da Apple é literal: ``\textit{TCP can be used
as the transport protocol for AFP version 2.1 and later}'' \cite{appleafptcp}. O que o \textbf{AFP
3.0} (Mac OS X Server 10.0.3, 2001) muda não é a \emph{existência} do transporte TCP, e sim sua
\emph{exclusividade}: a partir dele o AppleTalk deixa de transportar dados e fica restrito à
descoberta de serviço. Suas características determinantes, e o que elas explicam:
...
e a de maior consequência prática. O FCoE é um \textit{EtherType} próprio (\textbf{0x8906} para o
quadro de dados; \textbf{0x8914} para o FIP, o protocolo de inicialização) encapsulado diretamente
em quadro Ethernet: não há cabeçalho IP, e portanto \textbf{não há o que rotear}. O FCoE vive dentro de um domínio de camada 2. O iSCSI, por rodar sobre TCP/IP, atravessa
...
\textbf{NVMe-oF} (\textit{NVMe over Fabrics}) transporta comandos NVMe sobre FC
(\textbf{FC-NVMe}), TCP (\textbf{NVMe/TCP}) ou RDMA (\textbf{NVMe/RoCE}).
As filas paralelas pertencem ao modelo NVMe; SCSI não deve ser confundido com a fila
única por porta do AHCI. O transporte e a implementação também influenciam o desempenho.
