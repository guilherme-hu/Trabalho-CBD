...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Modelos de Pagamento:} Define como o recurso será faturado. Pode ser Sob Demanda (\emph{On-Demand}), onde se paga apenas pelo uso exato, ou por instâncias \emph{Spot} (capacidade ociosa com grande desconto, mas sujeita a interrupções abruptas, sendo inadequada para bancos de dados críticos). Para cargas previsíveis, utiliza-se Instâncias Reservadas ou \emph{Savings Plans}, ambos com descontos em troca de compromissos de 1 a 3 anos. A diferença é que as Reservadas fixam o cliente a um tipo de máquina específico, enquanto os \emph{Savings Plans} atrelam o compromisso a um gasto por hora, oferecendo flexibilidade para trocar de hardware ao longo do contrato.
\end{itemize}
Para garantir disponibilidade, a arquitetura utiliza o \emph{Auto Scaling}, que monitora métricas de uso e escala a infraestrutura \textbf{horizontalmente} --- criando novas instâncias e distribuindo o tráfego via \emph{Elastic Load Balancing} quando gatilhos de sobrecarga são atingidos. Para que essas instâncias tenham onde gravar dados de forma persistente, o EC2 depende quase que exclusivamente do Amazon EBS.
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Amazon EBS (Elastic Block Store):} Como visto anteriormente, o EBS atua como um armazenamento de blocos (disco rígido virtual) persistente e independente, sendo a escolha primária para SGBDs tradicionais. Ele resolve o problema do armazenamento efêmero nativo do EC2 (\emph{Instance Store}). O seu grande diferencial prático na AWS é a elasticidade \textbf{vertical}: o recurso \emph{Elastic Volumes} permite aumentar o tamanho do disco ou alterar sua capacidade de leitura/escrita (IOPS) em tempo real, sem desconectar o volume ou desligar a instância.
    
    \item \textbf{Amazon S3 (Simple Storage Service) e S3 Glacier:} Conforme introduzido anteriormente, o S3 é um armazenamento de objetos acessado via rede por API/SDKs e estruturado em \emph{buckets}, atuando como a fundação de \emph{Data Lakes} em vez de bancos transacionais. Para garantir sua altíssima durabilidade, a arquitetura padrão replica os objetos imutáveis em pelo menos três Zonas de Disponibilidade (AZs) distintas.
    
    O modelo de custos do ecossistema S3 varia conforme a classe de armazenamento (\emph{tier}), separando a cobrança entre a retenção do dado e a sua recuperação. A família \textbf{S3 Standard} utiliza três ou mais AZs e é ideal para dados acessados frequentemente (como imagens médicas de carregamento imediato); retém-se o dado a um custo maior, mas sem taxas extras de leitura. A família \textbf{S3 One Zone} difere por utilizar apenas uma AZ. Dentro dessas famílias há variações: o \emph{Express One Zone} entrega latência baixíssima por um custo de armazenamento maior, enquanto as opções \emph{IA (Infrequent Access)} reduzem o custo de retenção, mas embutem uma taxa financeira na recuperação. Por fim, a família \textbf{S3 Glacier} foca em arquivamento (como retenção legal de longo prazo) e divide-se em três opções: \emph{Instant Retrieval} (mantém acesso em milissegundos, mas com alta taxa financeira de leitura), \emph{Flexible Retrieval} (armazenamento mais barato, mas a recuperação leva de minutos a horas) e \emph{Deep Archive} (retenção com o menor custo possível, exigindo de 12 a 48 horas para recuperar os dados).
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Amazon RDS (Relational Database Service):} serviço de \emph{Database as a Service} (DBaaS) para bancos de dados relacionais tradicionais, suportando os motores MySQL, PostgreSQL, MariaDB, Oracle e SQL Server. O RDS automatiza tarefas operacionais como provisionamento, aplicação de \emph{patches} no motor de banco, backups automáticos com retenção configurável (snapshots diários combinados a logs de transação, permitindo restauração \emph{point-in-time}) e monitoramento. Para alta disponibilidade, oferece implantações \textbf{Multi-AZ}, nas quais uma réplica síncrona é mantida em outra \emph{Availability Zone} e promovida automaticamente em caso de falha do nó primário; para escalabilidade de leitura, suporta \textbf{Read Replicas} assíncronas.

    \item \textbf{Amazon Aurora:} motor relacional desenvolvido pela própria AWS, compatível com as APIs e o \emph{wire protocol} do MySQL e do PostgreSQL, permitindo migração sem alteração significativa da camada de aplicação. Sua principal diferença arquitetural é o desacoplamento entre a camada de computação e a camada de armazenamento: o armazenamento é distribuído, replicado em seis cópias ao longo de três \emph{Availability Zones} e escala automaticamente em incrementos de até 128 TB, sem necessidade de provisionamento manual. Isso resulta em maior taxa de transferência (\emph{throughput}), recuperação de falhas mais rápida e suporte a até 15 réplicas de leitura de baixa latência (contra 5 no RDS convencional). O Aurora também realiza \emph{backup} contínuo para o Amazon S3, sem impacto perceptível de desempenho, e oferece variantes como o \textbf{Aurora Serverless} (escalonamento automático de capacidade computacional conforme a demanda) e o \textbf{Aurora Global Database} (replicação entre regiões com baixa latência, voltada a aplicações globais e disaster recovery).

    \item \textbf{Amazon DynamoDB:} banco de dados NoSQL totalmente gerenciado, do tipo chave-valor/documento, sem esquema fixo --- cada item pode conter um número e tipo de atributos distintos. Sua origem remonta ao sistema interno \emph{Dynamo} (2004--2007), desenvolvido pela Amazon para resolver a indisponibilidade do carrinho de compras da Amazon.com em picos de tráfego, quando bancos relacionais tradicionais não sustentavam o volume de escritas concorrentes. O DynamoDB herda esses princípios de alta disponibilidade e escalabilidade horizontal, oferecendo latência de milissegundos de dois dígitos de forma consistente, independentemente do volume de dados armazenado. Suporta dois modos de capacidade (\emph{On-Demand}, com cobrança por uso, e \emph{Provisioned}, com capacidade de leitura/escrita reservada) e permite escolher entre consistência eventual ou consistência forte nas leituras. É indicado para cargas de trabalho de altíssima escala e baixa latência, como e-commerce, sistemas de sessão, jogos online e IoT, sendo o modelo de dados desenhado em função dos padrões de consulta da aplicação, e não da normalização relacional --- razão pela qual não é adequado para consultas com \emph{joins} complexos entre entidades.

    \item \textbf{Amazon Redshift:} serviço de \emph{Data Warehouse as a Service} (DWaaS), baseado em armazenamento colunar e processamento massivamente paralelo (\emph{Massively Parallel Processing}, MPP), voltado a consultas analíticas (OLAP) sobre grandes volumes de dados históricos. O armazenamento por colunas, combinado a técnicas de compressão (\emph{dictionary encoding}, \emph{run-length encoding}, entre outras), permite que operações de agregação leiam apenas as colunas relevantes para a consulta, em vez de linhas inteiras, reduzindo drasticamente o volume de I/O em relação a bancos orientados a linha. O Redshift foi lançado em 2012 com base na tecnologia da ParAccel, na qual a AWS investira, como resposta ao alto custo e à rigidez de soluções tradicionais de \emph{data warehousing} on-premises (Teradata, Oracle, IBM Netezza), oferecendo desempenho equivalente a uma fração do custo e sem necessidade de hardware proprietário. Integra-se nativamente ao Amazon S3 por meio do \textbf{Redshift Spectrum}, recurso que permite consultar diretamente dados armazenados no \emph{data lake} sem necessidade de carregá-los previamente no cluster, viabilizando uma arquitetura de \emph{lake house}. Tipicamente, o Redshift é alimentado por processos de ETL a partir de bancos operacionais (RDS, Aurora, DynamoDB), consolidando dados transacionais para fins de \emph{Business Intelligence} e relatórios agregados.
