...
\begin{itemize}[leftmargin=*]
    \item \textbf{Amazon EBS (Elastic Block Store):} O problema inicial da computação no EC2 é que o armazenamento nativo da máquina (\emph{Instance Store}) é efêmero: se a instância for encerrada, os dados desaparecem. O EBS resolve isso atuando como um "disco rígido virtual" persistente e independente, que é anexado pela rede à instância. É a escolha primária para a instalação de SGBDs tradicionais, pois suporta alta taxa de leitura e escrita (IOPS). Diferente do EC2, que escala adicionando mais máquinas, a elasticidade do EBS é \textbf{vertical}: o recurso \emph{Elastic Volumes} permite aumentar o tamanho do disco ou alterar seu desempenho em tempo real, sem desconectar o volume ou desligar a instância.
    \item \textbf{Amazon S3 (Simple Storage Service), Standard, One Zone e Glacier:} Diferente do EBS, o S3 não é um disco anexado a um servidor, mas um armazenamento de objetos acessado via rede por chamadas de API (HTTP) ou SDKs. Os dados não são organizados em uma hierarquia de pastas tradicionais, mas como objetos imutáveis (compostos por arquivo, metadados e uma chave de identificação) dentro de espaços planos chamados \emph{buckets}. Para garantir altíssima durabilidade física, a arquitetura padrão do S3 replica automaticamente cada objeto em pelo menos três Zonas de Disponibilidade (AZs) distintas. Por substituir o objeto inteiro em vez de editar pequenos blocos, não serve para rodar um banco transacional, mas atua como a fundação de baixo custo e alta escala para \emph{Data Lakes}.

O modelo de custos do ecossistema S3 varia conforme a classe de armazenamento (\emph{tier}), separando a cobrança entre a retenção do dado e a sua recuperação (acesso). A família \textbf{S3 Standard} utiliza três ou mais AZs e é ideal para dados acessados frequentemente (como imagens médicas de carregamento imediato); retém-se o dado a um custo maior, mas sem taxas extras de leitura. A família \textbf{S3 One Zone} difere por utilizar apenas uma AZ. Dentro dessas famílias há variações: o \emph{Express One Zone} entrega latência baixíssima por um custo de armazenamento maior, enquanto as opções \emph{IA (Infrequent Access)} reduzem o custo de retenção, mas embutem uma taxa financeira na recuperação. Por fim, a família \textbf{S3 Glacier} foca em arquivamento (como retenção legal de longo prazo) e divide-se em três opções: \emph{Instant Retrieval} (mantém acesso em milissegundos, mas com alta taxa financeira de recuperação), \emph{Flexible Retrieval} (armazenamento mais barato, mas a recuperação leva de minutos a horas) e \emph{Deep Archive} (retenção com o menor custo possível, exigindo de 12 a 48 horas para recuperar os dados).
\end{itemize}
