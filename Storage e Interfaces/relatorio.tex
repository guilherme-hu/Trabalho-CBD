...
\item \textbf{Desdobramos ``Flash Memory --- SSD'' em três linhas} (consumidor PCIe~5.0,
      ...
      categoria única do livro esconde uma variação de \textbf{cerca de duas ordens de grandeza em
...
\noindent{\footnotesize\textbf{Leitura da coluna de 2014:} é o \emph{topo} de cada faixa da
Tabela~16.1 original --- ``Main Memory-RAM 4GB--1TB'', ``Flash Memory-SSD 64 GB--1TB'',
``Flash Memory-USB stick 4GB--512GB'', ``Magnetic Disk 400 GB--8TB'', ``Magnetic Tape
2.5TB--8.5TB'' (Elmasri \& Navathe, Tabela~16.1, ref.~[1]). Comparamos topo com topo; comparar a
média seria mais fiel, mas o livro não publica distribuição.\par}

\medskip
Os tempos de acesso mantidos do livro não permitem medir uma evolução de latência.
Capacidade e banda publicadas podem ser comparadas; uma conclusão sobre latência exige
medições equivalentes de carga, fila e percentil.
...
convergisse por volta de 2026--2028. \textbf{Alcance da afirmação:} é a leitura de projeções de
mercado amplamente repetidas na imprensa técnica, e \textbf{não fixamos uma projeção nomeada e
datada} que a sustente --- registramos a lacuna em vez de atribuí-la genericamente ``à
literatura''. O que \emph{está} documentado, e é o ponto, é o desfecho: \textbf{o oposto ocorreu.} A razão é estrutural: a
...
A razão precisa ser \textbf{datada}, porque é extremamente volátil. O \emph{Flash Volatility
Index} da VDURA (ref.~[29]) registra a evolução trimestral do prêmio do SSD corporativo sobre o
HDD \emph{nearline}:
...
Os 16,3$\times$ referem-se ao trimestre anterior. \textbf{Ressalva de proveniência:} os valores
de 18,6$\times$, 23,2$\times$ e 7,0$\times$ estão na publicação de 11/ago./2026 da VDURA
(ref.~[29]); o de 16,3$\times$ vem de divulgação trimestral anterior do mesmo índice, e o grupo
\textbf{não fixou o comunicado específico} --- quem for reapresentar esta tabela deve fechar essa
célula antes. Uma versão anterior deste
...
Preço por capacidade não é custo total de propriedade. A comparação não inclui
energia, manutenção, redundância nem infraestrutura de rede; em mídia removível,
o preço do cartucho/disco é separado do leitor. Não inferimos tendências de mercado
a partir de uma consulta pontual.
...
\item SEAGATE. \emph{Exos M SATA Product Manual, 32/30/28/24\,TB}, Rev.~D, ago./2025 ---
      sustenta os pontos de 24 a 32\,TB. O ponto de \textbf{36\,TB} \emph{não} consta desse
      manual e vem de fonte separada: SEAGATE. \emph{Seagate Introduces Hard Drive Capacities of
      Up to 36TB, Extending Its HAMR-Based Mozaic 3+ Technology Platform}, 20/jan./2025.
...
\item BROADCOM. \emph{Brocade G720 Switch Product Brief}. Texto literal:
      ``\emph{Latency for locally switched ports is 460 ns (including FEC)}'' --- o valor vale
      para \textbf{portas comutadas localmente}, não para um caminho porta a porta arbitrário,
      que soma travessia de ISL. \url{https://docs.broadcom.com/doc/G720-Switch-PB}.
...
\item TRENDFORCE. Relatórios de preço de contrato de DRAM e NAND, 1T26 a 3T26.
      \url{https://www.trendforce.com/price}. \textbf{Ressalva:} consultados como série de
      mercado; não reproduzimos valores pontuais deste provedor no corpo do texto.
...
\item UBICLOUD. \emph{PostgreSQL performance: local vs.\ network-attached storage} (TPC-C e
      TPC-H comparando NVMe local, Amazon Aurora e RDS). \url{https://www.ubicloud.com/blog}.
      \textbf{Ressalva:} medição de fornecedor sobre a própria plataforma; a degradação de
      4,6$\times$ citada na Seção~7 (Q6) deve ser lida com essa origem em vista.
...
\item \textbf{Medição de latência de expansores CXL 2.0} (214--271\,ns contra 111--117\,ns de
      DDR5 local, nota~$\dagger$ da Tabela~\ref{tab:161}). \textbf{Lacuna declarada:} os valores
      vêm de medição independente revisada por pares, mas \textbf{este trabalho não fixou a
      referência exata}. Enquanto ela não for fixada, os 214--271\,ns devem ser lidos como ordem
      de grandeza, como já adverte a própria nota, e não como dado com proveniência fechada.
\end{enumerate}
\endgroup
