...
A Tabela~\ref{tab:161} é apresentada a seguir, em orientação paisagem. Ela deve ser
lida junto com as notas metodológicas que a seguem, nas quais está registrada a origem de cada
...
de \textbf{607 mil vezes} entre o topo e a base ($199 \div 67{.}108{,}864$ contra
...
acesso, largura de banda segregada em leitura e escrita, preços de referência e preço por
kilobyte decimal. Consulta original: 05/09/2026; MSRP é histórico, não cotação atual. Fontes por linha nas notas que seguem a
...
\item \textbf{Preço por KB --- método.} Usamos 1\,KB = 1.000 bytes em todas as linhas.
      Para memória, convertemos a capacidade binária em bytes antes da divisão.
      Exemplo: 64\,MiB = 67.108,864\,KB; US\$\,199 / 67.108,864\,KB =
      $2{,}97\times10^{-3}$ US\$/KB. O preço/KB usa a capacidade e o preço pontuais de
      \texttt{dados.py}; não combina os extremos das faixas de modelos.

\item \textbf{O preço da SRAM é um \emph{proxy}, não um preço.} Cache SRAM não é vendida
      como produto; o valor acima é o \emph{delta} de preço entre duas CPUs comparadas com e
      ...
      $\approx$72$\times$ o preço por byte da DRAM de servidor nesta base.
...
\item \textbf{Tempo de acesso de SSD.} Sem valor confirmado para o modelo e a carga,
      usamos os 50\,\textmu s da Tabela 16.1 original, conforme o enunciado.
      Isso vale para o 9100 PRO, D5-P5336 e Micron 9550 nesta comparação;
      não é medição desses produtos. Foram retirados os números não confirmados
      de latência típica e percentil do D5-P5336.
...
\item \textbf{Tempos de acesso mantidos do livro.} Sem valor confirmado de acesso total,
      mantemos HDD: 10\,ms; USB: 100\,\textmu s; óptico: 180\,ms.
      A latência rotacional isolada não equivale ao acesso total do HDD.
...
\item \textbf{Os números do 9100 PRO são do modelo de 1\,TB}, cuja capacidade e MSRP oficial
      de lançamento sem dissipador (US\$\,199,99) formam a base de cálculo da coluna de preço/KB. O
...
\subsection{Limitações dos dados comerciais}

As linhas de CXL (complementar) e biblioteca de fita não têm preço público confirmado;
por isso não participam da comparação de custo. A biblioteca foi mantida por corresponder
ao \textit{tape jukebox} do livro. Não se deve interpretar ``n/d'' como custo zero.
O MSRP histórico do SSD Samsung não substitui uma cotação atual. As faixas comerciais
registradas na consulta original precisam de fonte por modelo para auditoria de preços;
os cálculos verificam a aritmética, não comprovam disponibilidade ou validade da oferta.

\subsection{Justificativa das linhas acrescentadas e removidas}

Acrescentamos a linha de memória CXL 2.0 porque a hierarquia de 2026 tem um degrau que não
existia em 2014 e que é o único candidato a ocupar o vazio deixado pelo Optane; sem ela, o salto de
70\,ns para 50\,\textmu s ficaria inexplicado. Desdobramos a linha ``Flash Memory --- SSD'' do livro
em três, uma de consumidor PCIe~5.0, uma de datacenter QLC de alta capacidade e uma de datacenter TLC
de alto desempenho, porque a categoria única esconde uma variação de cerca de duas ordens de grandeza
em capacidade, de 1\,TB a 122,88\,TB, e de mais de 5$\times$ em preço por byte, e porque escolher
entre QLC de capacidade e TLC de desempenho é uma decisão de projeto para um SBD. Desdobramos também
a fita em LTO-9 e LTO-10, já que a transição está em curso: o LTO-10 foi lançado em 2025 com 30\,TB e
teve o cartucho de 40\,TB especificado em novembro de 2025, com embarques a partir de janeiro de 2026.

Duas linhas ficaram de fora. O Intel Optane seria a que faria mais sentido teórico, mas não está
comercialmente disponível, o que viola o critério C2, e por isso registramos sua ausência como achado
e não como omissão. O Sony Optical Disc Archive saiu pela mesma razão, já que a linha foi encerrada em
todas as regiões em 31 de março de 2025.

\subsection{Análise: o que mudou entre 2014 e 2026}

\subsubsection*{(a) Evolução de capacidade}
...
\noindent{\footnotesize A coluna de 2014 é o topo de cada faixa da Tabela~16.1 original, que
publica ``Main Memory-RAM 4GB--1TB'', ``Flash Memory-SSD 64 GB--1TB'', ``Flash Memory-USB stick
4GB--512GB'', ``Magnetic Disk 400 GB--8TB'' e ``Magnetic Tape 2.5TB--8.5TB'' (Elmasri \& Navathe,
ref.~[1]). Comparamos topo com topo, já que comparar a média seria mais fiel mas o livro não publica
distribuição.\par}

\medskip
Os tempos de acesso mantidos do livro não permitem medir uma evolução de latência: capacidade e banda
publicadas podem ser comparadas, mas uma conclusão sobre latência exigiria medições equivalentes de
carga, fila e percentil, que o livro não traz.

\subsubsection*{(b) Evolução de largura de banda}

\begin{figure}[H]\centering
\includegraphics[width=0.92\textwidth]{fig/fig2_banda.pdf}
\caption{Largura de banda de leitura e escrita por tecnologia, em escala logarítmica. O SSD
NVMe PCIe~5.0 entrega 20$\times$ a banda que a Tabela~16.1 original registra para SSD, ou seja,
750\,MB/s contra 14,7\,GB/s, enquanto o disco magnético avançou 1,4$\times$ no mesmo período. Vale
notar que a fita LTO ultrapassa o disco magnético em taxa sequencial, resultado contraintuitivo que
decorre de a fita não precisar posicionar um braço entre blocos consecutivos.}
\end{figure}

\subsubsection*{(c) SSD e HDD divergiram em vez de convergir}

Projeções de mercado repetidas na imprensa técnica esperavam que o preço por terabyte de SSD e
HDD convergisse por volta de 2026 a 2028, e convém registrar que não fixamos uma dessas projeções com
autor e data, de modo que a expectativa entra aqui como contexto e não como fonte. O que está
documentado é o desfecho, e ele foi o contrário: a demanda de IA consumiu a capacidade fabril de
NAND, parte dos compradores migrou de SSD de volta para HDD, e isso pressionou também o preço do
disco.

Como a razão entre os dois é volátil, ela só faz sentido com data. O \emph{Flash Volatility Index} da
VDURA (ref.~[29]) acompanha trimestralmente o prêmio do SSD corporativo sobre o HDD \emph{nearline}:
...
Em agosto de 2026, um SSD TLC corporativo de 30\,TB custava cerca de US\$\,22.600, ou
US\$\,753/TB, contra cerca de US\$\,1.216 de um HDD de mesma capacidade, ou US\$\,41/TB. Os
18,6$\times$ são a observação parcial consultada em 05/09/2026 e não devem ser apresentados como
fechamento do terceiro trimestre, enquanto os 16,3$\times$ se referem ao trimestre anterior. Quanto à
proveniência da tabela, os valores de 18,6$\times$, 23,2$\times$ e 7,0$\times$ estão na publicação de
11 de agosto de 2026 da VDURA (ref.~[29]), mas o de 16,3$\times$ vem de divulgação trimestral
anterior do mesmo índice, cujo comunicado específico o grupo não chegou a fixar: quem for
reapresentar esta tabela deve fechar essa célula antes. Uma versão anterior deste trabalho citava
``aproximadamente 16$\times$'' sem carimbo de trimestre, erro registrado como E15 na Seção~9.4, e a
lição vale além do número, porque num mercado que oscilou de 7$\times$ a 23$\times$ em quatro
trimestres uma razão sem data não informa nada.

\subsubsection*{(d) Limite da comparação econômica}

Preço por capacidade não é custo total de propriedade, e a comparação acima não inclui energia,
manutenção, redundância nem infraestrutura de rede; em mídia removível, o preço do cartucho ou do
disco ainda é separado do preço do leitor. Por isso não inferimos tendências de mercado a partir de
uma consulta pontual.

\subsubsection*{(e) Síntese}
...
Antes de qualquer comparação, é preciso fixar a aritmética, porque três fatores sucessivos
separam a ``velocidade'' anunciada de uma interface da taxa de dados que ela efetivamente entrega.

O primeiro é a \textbf{unidade}: como 1\,GB/s equivale a 8\,Gb/s, uma interface de 6\,Gb/s nunca
entrega 6\,GB/s. O próprio Silberschatz comete esse deslize na Seção~12.2 ao escrever que ``\emph{the
SATA-3 version of SATA nominally supports 6 gigabytes per second}'', quando são 6 gigabits por
segundo, como a frase seguinte do livro reconhece ao citar 600\,MB/s. Vale notar que a mesma frase
usa a unidade correta logo adiante, ao dizer que ``\emph{SAS version 3 supports data transfer rates
of 12 gigabits per second}'', o que mostra tratar-se de lapso e não de convenção do texto.

O segundo fator é a \textbf{codificação de linha}. Todo enlace serial insere bits de controle para
manter o balanceamento DC e permitir a recuperação do relógio a partir do próprio fluxo de dados, e a
tabela a seguir reúne as codificações usadas pelas interfaces deste estudo.
...
FLIT 242B/256B & 94,53\% & 5,8\% & PCIe 6.0 / 7.0 (FLIT mode $+$ RS-FEC $+$ CRC) \\
\bottomrule
\end{tabular}
\end{center}

\noindent{\footnotesize\textbf{Base do \emph{overhead}:} bits acrescentados $\div$ bits de
carga útil (8b/10b: $2/8 = 25{,}0\%$; 64b/66b: $2/64 = 3{,}1\%$; 128b/130b: $2/128 = 1{,}6\%$;
128b/132b: $4/128 = 3{,}1\%$; 256b/257b: $1/256 = 0{,}4\%$; 128b/150b: $22/128 = 17{,}2\%$;
FLIT: $14/242 = 5{,}8\%$). Sobre o quadro total os percentuais seriam menores --- é a mesma
armadilha de rótulo da Seção~4.1(1), aplicada a si mesma.\par}

\medskip
O terceiro fator é a \textbf{modulação}. O NRZ transmite 1 bit por símbolo, enquanto o PAM4
transmite 2, dobrando a taxa sem dobrar a frequência de sinalização, ao custo de reduzir a margem de
ruído a um terço, o que torna a correção de erro obrigatória. O PAM4 é usado em FC 64/128GFC,
InfiniBand HDR/NDR/XDR, PCIe 6.0/7.0 e Ethernet 400G, e o USB4 v2.0 usa PAM3, com três níveis e cerca
de 1,58 bit por símbolo.

Aplicando os três fatores, o SATA 3.0 entrega 6,0\,Gb/s brutos que, multiplicados por 8/10 da
codificação e divididos por 8, dão os 600\,MB/s de sua especificação; e o PCIe 5.0 em $\times$4
entrega 32\,GT/s por \textit{lane} que, multiplicados por 4 \textit{lanes} e por 128/130, e divididos
por 8, dão 15,754\,GB/s.
...
de grandeza, sem publicação oficial de organismo normatizador. \textbf{Duas convenções de
leitura:} (i) na família \emph{Fibre Channel}, a coluna ``Ano'' é a \textbf{disponibilidade de
mercado} segundo o \emph{roadmap} da FCIA, não o ano de conclusão da norma --- o FC-PI-8
(128GFC) fechou em 2023, mas a produtização é de $\approx$2025; (ii) as taxas de FC estão
\textbf{por direção}, e a FCIA publica também o valor \emph{full-duplex}, que é o dobro --- citar
``128GFC'' sem dizer qual das duas escalas erra por $2\times$.}\label{tab:if}\\
\toprule
...
PCIe 3.0 $\times$4 / $\times$16 & 2010 & Serial (\emph{lanes}) & 8\,GT/s por \emph{lane} & 3,94 / 15,75\,GB/s & $<$1\,\textmu s & $\approx$0,3\,m (PCB) & Depende & Árvore ponto-a-ponto & SSD de geração anterior \\
PCIe 4.0 $\times$4 / $\times$16 & 2017 & Serial & 16\,GT/s & 7,88 / 31,51\,GB/s & $<$1\,\textmu s & $\approx$0,3\,m & Depende & idem & SSD de datacenter \\
PCIe 5.0 $\times$4 / $\times$16 & 2019 & Serial & 32\,GT/s & 15,75 / 63,02\,GB/s & $<$1\,\textmu s & $\approx$0,25\,m & Depende & idem & \textbf{Padrão atual} \\
PCIe 6.0 $\times$4 / $\times$16 & 2022 & Serial PAM4 & 64\,GT/s & 30,25 / 121\,GB/s & $<$1\,\textmu s ($+$FEC) & curta & Depende & idem & Fronteira (Micron 9650) \\
PCIe 7.0 $\times$4 / $\times$16 & jun./2025 & Serial PAM4 & 128\,GT/s & 60,5 / 242\,GB/s & $<$1\,\textmu s & curta & Depende & idem & Ainda sem produtos \\
NVMe (local) & 2011--2026 & Protocolo s/ PCIe & --- & $=$ PCIe & \textbf{20--70\,\textmu s} & --- & Depende & 65.535 filas $\times$ 65.536 cmds & \textbf{OLTP, \emph{redo log}} \\
\midrule
...
iSCSI & 2004; norma vigente RFC 7143 (2014) & Protocolo s/ IP & 1--100\,GbE & 110\,MB/s--11\,GB/s & \textbf{500--800\,\textmu s} & ilimitada (IP) & Sim & Rede IP & PME, virtualização \\
\midrule
...
32GFC (Gen 6) & 2016 & Serial 256b/257b & 28,05\,GBd & \textbf{3.200\,MB/s} por direção & 460\,ns/\emph{switch} & 100\,m OM4 / 10\,km & Sim & idem & \textbf{SAN corporativa} \\
64GFC (Gen 7) & 2020 & Serial PAM4 & 28,9\,GBd $\times$2 & \textbf{6.400\,MB/s} por direção & 460\,ns/\emph{switch} & 100\,m OM4 / 10\,km & Sim & idem & SAN de alto desempenho \\
128GFC (Gen 8) & $\approx$2025 & Serial PAM4 & 56,1\,GBd $\times$2 & \textbf{12.425\,MB/s} por direção & idem & 100\,m OM4/OM5 / 10\,km & Sim & idem & Núcleo de SAN \\
...
As linhas de fatores de forma (M.2, U.2, U.3 e EDSFF) e de HBA são de natureza distinta das
demais e estão na tabela porque constam da lista do enunciado, mas convém marcar que elas são
conectores e adaptadores, e não protocolos: um M.2 não tem taxa própria, já que expõe as mesmas
\textit{lanes} PCIe da linha correspondente, e é por isso que suas colunas de taxa trazem
``$=$~PCIe''. O que distingue essas linhas são as colunas de \textit{hot-plug}, alimentação e
mecânica, onde estão as decisões de projeto que importam: o M.2 não suporta \textit{hot-swap} e opera
só em 3,3\,V, o que limita a dissipação a cerca de 8 a 11\,W e o inviabiliza para SSDs PCIe~5.0/6.0
de datacenter, que passam de 25\,W, enquanto o EDSFF suporta \textit{hot-swap}, 12\,V e até 70\,W.
...
sobre ele. \textbf{Atenção ao eixo:} aqui o ano é o de \emph{publicação da especificação}; na
Tabela~\ref{tab:if} o ano da família \textit{Fibre Channel} é o de \emph{disponibilidade de
mercado}. As duas bases diferem em até três anos e não devem ser lidas uma contra a outra.}
\end{figure}
...
O grupo registra abaixo as questões que considera em aberto e que gostaria de levar ao debate em
sala.

\begin{enumerate}[label=\textbf{Q\arabic*.}]
\item O modelo de custo do otimizador deveria ser calibrado automaticamente? Se o
      ...
      administrador que talvez nem saiba que o parâmetro existe?
\item Faz sentido continuar ensinando o modelo de I/O baseado em contagem de blocos, ou o
      modelo correto para o hardware de 2026 seria baseado em latência\,$\times$\,paralelismo,
      contabilizando a profundidade de fila explicitamente?
\item A descontinuação do Optane foi uma falha de mercado ou de tecnologia? O 3D~XPoint
      funcionava e ocupava um degrau real da hierarquia, de modo que, se a razão foi custo de
      fabricação, fica a pergunta sobre o que impede que a mesma lacuna se reabra indefinidamente.
\item O CXL substituirá o \emph{buffer pool} ou o \emph{tier} de SSD? Se a memória barata
      escalar via CXL, bancos ``in-memory'' de dezenas de terabytes ficam viáveis, e cabe perguntar
      o que mudaria no projeto de um SGBD que hoje assume que os dados não cabem na memória.
\item Em que ponto a latência de cauda deveria entrar no modelo formal? Toda a literatura de
      otimização usa custo esperado, isto é, média, enquanto os SLAs reais são escritos em P99 e
      P99,9. Existe formalismo de otimização de consultas que otimize percentil em vez de média?
\item Armazenamento em nuvem é aceitável para o \emph{tier} 1? Silberschatz observa que a
      nuvem tem latência de dezenas a centenas de milissegundos quando os dados não são
      colocalizados, e os dados de TPC-C que reunimos confirmam degradação de 4,6$\times$, o que
      deixa em aberto se o limite é físico ou de arquitetura.
\item O enunciado da tarefa continha um erro factual, ao afirmar que ``SATA também é conhecida
      como NL-SAS'', afirmação que vem do próprio Elmasri \& Navathe (§16.2.1). O grupo optou por
      documentar a correção em vez de reproduzi-la, e fica a pergunta sobre se essa foi a conduta
      correta em um trabalho avaliativo.
\end{enumerate}
...
\item LTO PROGRAM. \emph{LTO-9 Technical Brief}; \emph{LTO Program Announces New 40\,TB
      LTO-10 Cartridge Specifications and Refreshes its Roadmap}, 12/nov./2025 (40\,TB nativos,
      até 100\,TB a 2,5:1; embarques no 1\textsuperscript{o} trimestre de 2026; \emph{roadmap}
      revisado com pico de 913\,TB comprimidos na Geração~14 --- os 365\,TB nativos citados na
      Seção~3.4 são a derivação $913 \div 2{,}5$).
...
\item AMD. \emph{Ryzen 9 9950X3D2 Dual Edition --- Specifications}.
      ...
      Consulta: 05/09/2026.
\item SEAGATE. \emph{Exos M SATA Product Manual, 32/30/28/24\,TB}, Rev.~D, ago./2025, que
      sustenta os pontos de 24 a 32\,TB. O ponto de 36\,TB não consta desse manual e vem de fonte
      separada: SEAGATE. \emph{Seagate Introduces Hard Drive Capacities of Up to 36TB, Extending
      Its HAMR-Based Mozaic 3+ Technology Platform}, 20/jan./2025.
...
\item WESTERN DIGITAL. \emph{NVMe-oF Network Storage Protocol: NVMe/TCP vs.\ RDMA with
      RoCEv2} (OpenFlex Data24). Condições declaradas no documento: aleatório, bloco de 4\,K,
      1 \emph{job}, QD1, 20 min, ``\emph{measured at the 4 nines (0.9999)}''. Valores:
      RoCE escrita 41,22\,\textmu s, RoCE leitura 162,82\,\textmu s, TCP escrita
      122,37\,\textmu s, TCP leitura 177,15\,\textmu s.
...
\item BROADCOM. \emph{Brocade G720 Switch Product Brief}, que registra literalmente
      ``\emph{Latency for locally switched ports is 460 ns (including FEC)}''. O valor vale para
      portas comutadas localmente, e não para um caminho porta a porta arbitrário, que ainda soma a
      travessia de ISL. \url{https://docs.broadcom.com/doc/G720-Switch-PB}. \emph{eHBA 9600-24i};
      \emph{Emulex LPe38102} (64GFC).
...
\item TRENDFORCE. Relatórios de preço de contrato de DRAM e NAND, 1T26 a 3T26.
      \url{https://www.trendforce.com/price}. Consultados como série de mercado; não reproduzimos
      valores pontuais deste provedor no corpo do texto.
\item VDURA. \emph{Flash Volatility Index} --- ``SSD Prices Settle Into a Costly New Normal at
      $\sim$6.5x Year-Ago Levels'', 11/ago./2026. Fonte primária dos valores de 18,6$\times$,
      23,2$\times$ (pico do 1T26), 7,0$\times$ (3T25) e dos preços de US\$\,22.600 (SSD TLC
      30\,TB) e US\$\,1.216 (HDD 30\,TB) usados na Seção~3.5(c).
...
\item MICROSOFT AZURE. \emph{PostgreSQL on AKS with local NVMe}, blog de engenharia,
      jul./2025. \url{https://techcommunity.microsoft.com/category/azure/blog/adforpostgresql}
\item UBICLOUD. \emph{PostgreSQL performance: local vs.\ network-attached storage} (TPC-C e
      TPC-H comparando NVMe local, Amazon Aurora e RDS). \url{https://www.ubicloud.com/blog}. É
      medição de fornecedor sobre a própria plataforma, e a degradação de 4,6$\times$ citada na
      Seção~7 deve ser lida com essa origem em vista.
\item SCSI TRADE ASSOCIATION (STA) / SNIA. \emph{SAS Technology Roadmap} e declarações
      sobre 24G$+$ e SAS-5. \url{https://www.snia.org/forums/sfa}

\item Medição de latência de expansores CXL 2.0 (214--271\,ns contra 111--117\,ns de DDR5
      local, nota~$\dagger$ da Tabela~\ref{tab:161}). Os valores vêm de medição independente
      revisada por pares, mas este trabalho não fixou a referência exata, e por isso os
      214--271\,ns devem ser lidos como ordem de grandeza, e não como dado com proveniência
      fechada.
\end{enumerate}
\endgroup
...
--- distinguindo dados de fonte, estimativas e lacunas.
Na revisão editorial solicitada por Raphael em 08/09/2026, a IA padronizou KB,
corrigiu o MSRP, retirou latências não confirmadas e enxugou slides. Esse registro
não representa nova confirmação humana das fontes comerciais.
...
Seção 5 (gerente de armazenamento); coordenação da consolidação do Post-Mortem em 06/09/2026,
com conferência executada pelo Codex &
Decidiu ancorar a discussão em parâmetros verificáveis de SGBD real
(\texttt{random\_page\_cost}, \texttt{effective\_io\_concurrency}) e separar os problemas da
versão auditada das pendências que ainda existem nos artefatos atuais. \\

Guilherme En Shih Hu &
Rodada de verificação adversarial; execução do verificador e revisão final dos artefatos &
Decidiu manter uma segunda rodada independente para encontrar contradições internas e só fechar a
entrega depois que as contas e os termos-chave coincidissem entre relatório e slides. \\
\bottomrule
\end{tabular}
\end{center}
...
A pedido de Raphael Henrique da Silva Pereira, o Codex confrontou o plano de correções da versão anterior com
\texttt{dados.py}, \texttt{relatorio.tex}, os artefatos atuais e o verificador. Os ajustes
quantitativos já incorporados foram mantidos como histórico de revisão; a identificação e a
confirmação dos demais integrantes continuam pendentes e não foram inferidas.}
...
método e sua limitação. & Gabriel Schmitz, ao tentar reproduzir a conta. \\
...
Gabriel Schmitz, ao verificar $199/64\,\text{MiB}$. \\
...
transição explicada. & Gabriel Schmitz. \\
...
Vivian Maria, por suspeita levantada na leitura de Elmasri §16.11. \\
...
\textbf{faixa}, com registro explícito da divergência na Nota N9. & Bernardo Brandão. \\
...
(estimados) para citação acadêmica. & Enzo de Carvalho. \\
...
& Verificação adversarial (Seção~9.5); confirmado por Gabriel Schmitz contra o
...
E18 & Confundiu latência típica e QoS do D5-P5336. &
A revisão editorial retirou os valores não confirmados. Adotados 50\,\textmu s da
Tabela 16.1 original, explicitamente como referência do livro, não dado do produto.
& Revisão assistida por IA; sem nova atribuição de confirmação humana. \\
